...
const langLabelMassageFunctions = {
	ja: (value, isUI) => {
		...
		if (isUI && value.indexOf(' ') > -1)
			return value
				.split(/ +/g)
				.map((v) => `<span class="nowrap">${v}</span>`)
				.join('');
		return value;
	}
};
...
function onUpdateConnectionStatus(status: number) {
	if (status === 1 && config.privateMode) status = config.singleplayerMode ? 4 : 3;
	...
	if (connStatus !== undefined && (!status || status === 2))
		setTimeout(function () {
			if (connStatus === status) updateStatusText();
		}, 500);
	...
	if (status === 1 || status === 3 || status === 4) {
		...
		updatePlayerFriends();
		updateJoinedParty();
		if (eventPeriodCache) updateEvents();
		syncPrevLocation();
	} else {
		playerUuids = {};
		clearPlayerList();
...
export function fetchAndUpdatePlayerInfo(forLogin) {
	...
	navigator.serviceWorker?.getRegistration('/').then((registration) => {
		if (!registration) console.warn('updating player info but no service workers found');
	...
	apiFetch('info')
		...
		.then((jsonResponse) => {
			if (jsonResponse.uuid) {
				...
				if (fsBadgesButton) fsBadgesButton.classList.toggle('hidden', !loggedIn);
				if (jsonResponse.name) playerName.set(jsonResponse.name);
				syncPlayerData(
					jsonResponse.uuid,
					jsonResponse.rank,
				...
				const updateParty = () => {
					if (document.querySelector('#chatboxTabParties.active')) updatePartyList();
					else fetchAndUpdateJoinedPartyId();
				};
				if (isLogin && loggedIn) {
					initSessionWs().then(() => {
						...
						updateBadges(() => {
							...
							if (!eventPeriodCache) updateEventPeriod();
						});
						if (eventPeriodCache) updateEvents();
					...
					initSessionWs().then(() => {
						trySetChatName(loggedIn ? get(playerName) : '');
						updatePlayerFriends();
						updateParty();
...
function updatePlayerCount(count) {
	...
	if (localizedMessages)
		playerCountLabel.innerHTML = getMassagedLabel(
			localizedMessages.playersOnline[count == 1 ? 'singular' : 'plural'].replace('{COUNT}', count),
			true
		);
	if (playerCount === undefined) document.getElementById('onlineInfo').classList.remove('hidden');
	playerCount = count;
}
...
export function updateMapPlayerCount(count: number) {
	...
	if (localizedMessages)
		mapPlayerCountLabel.innerHTML = getMassagedLabel(
			localizedMessages.playersInMap[count == 1 ? 'singular' : 'plural'].replace('{COUNT}', count),
			true
		);
}
...
if (isBrowser) {
	...
	gameLogoImg.onload = function () {
		...
		gameLogo.setAttribute(
			'style',
			`background-image: url('${gameLogoUrl}'); width: ${width}px; height: ${height}px;`
		);
		document
			.getElementById('gameLogoOverlay')
			.setAttribute(
				'style',
				`-webkit-mask-image: url('${gameLogoUrl}'); mask-image: url('${gameLogoUrl}'); mix-blend-mode: ${gameLogoBlendModeOverrides[gameId] || 'multiply'};`
			);
...
function onLoadMap(mapName) {
	if (mapCheckTimer) clearTimeout(mapCheckTimer);
	mapCheckTimer = setTimeout(() => {
		...
		if (mapIdMatch) {
			...
			if (mapId === cachedMapId || ignoredMapIds.indexOf(mapId) > -1) return;
	...
	if (localizedMapLocations) {
		...
		if (locationsUpdated) {
			...
			if (is2kki) {
				...
				else queryAndSetWikiMaps(locations);
			} else locationNames = [];
...
export function updateLocationDisplay(locationHtml, colors) {
	if (!globalConfig.locationDisplay) return;
...
function hideLocationDisplay(fast = false) {
	if (fast) locationDisplayContainer.classList.add('fast');
...
function syncPrevLocation() {
	...
	const prevLocationsStr = cachedPrev2kkiLocations?.length
...
function onPlayerTeleported(mapId, x, y) {
	...
	if (ignoredMapIds.indexOf(mapIdStr) > -1) return;
...
function onReceiveInputFeedback(inputId) {
	if (inputId) {
		...
		if (configKey) {
			...
			if (isGlobal) globalConfig[configKey] = toggled;
			else config[configKey] = toggled;
...
export function openModal(
	...
	if (modalData) {
		for (let k of Object.keys(modalData)) modal.dataset[k] = modalData[k];
	}
...
export function closeModal() {
	...
	if (modalFadeOutContainer.childElementCount) return;
	const modalContainer = document.getElementById('modalContainer');
	...
	setModalUiTheme('confirmModal', config.uiTheme === 'auto' ? getSystemName() : config.uiTheme);
	...
	if (modalContainer.dataset.lastModalId) {
		...
		if (lastModalIdSeparatorIndex === -1)
			...
			openModal(
				modalContainer.dataset.lastModalId.slice(lastModalIdSeparatorIndex + 1),
				modalContainer.dataset.lastModalTheme.slice(lastModalThemeSeparatorIndex + 1)
			);
		}
	}
}
...
export function setPlayersTab(tab, saveConfig) {
	...
	if (tabIndex !== activeTabIndex) {
		for (let playersTab of document.getElementsByClassName('playersTab')) {
			...
			if (active || !tabIndex) playersTab.classList.remove('unread');
		}
...
export function updateYnomojiContainerPos(isScrollUpdate, chatInput) {
	...
	if (expandDown) {
		...
		ynomojiContainer.style.bottom =
			hasTouchscreen && ((isWrapped && isDownscale2) || isFullscreenSide)
	...
	ynomojiContainer.style.maxHeight =
		hasTouchscreen && ((isWrapped && isDownscale2) || isFullscreenSide)
...
export function onUpdateChatboxInfo() {
	...
	if (!layout.classList.contains('immersionMode') && !document.fullscreenElement && hasFlexWrap) {
		...
		if (chatboxInfo.offsetHeight >= 72) chatboxInfo.setAttribute('style', '');
	...
	if (document.fullscreenElement) {
		...
		if (window.innerWidth > 1050 || window.innerHeight < 595) {
			...
			if (unusedHeight >= 376 && showChat) {
				...
				chatboxContainerMarginTop = '24px';
				if (showChat) chatboxOverlap = true;
			}
		}
	} else {
...
function setFullscreenControlsHideTimer() {
	...
	fullscreenControlsTimer = setTimeout(function () {
		if (!document.querySelector('#controls button:hover') || hasTouchscreen) toggleControls(false);
		...
		.then(function (jsonResponse) {
			...
			if (localizedVersion) {
				...
				if (versionElement && versionMeta)
					versionElement.innerHTML = getMassagedLabel(
						localizedVersion.label.replace('{VERSION}', getLocalizedVersion(versionMeta.content))
					);
			}
			...
			if (isInitial) {
				...
				if (connStatus !== undefined) onUpdateConnectionStatus(connStatus);
				if (playerCount !== undefined) updatePlayerCount(playerCount);
			}
			...
			gameSelects.forEach((select) => {
				Object.keys(localizedMessages.games).forEach((game) => {
					...
					if (matchingOption) matchingOption.innerText = localizedMessages.games[game];
				});
			});
			...
			const initLocationsCallback = () => {
				...
				if (!isInitial) {
					...
					const locationMessages = document.querySelectorAll(
						'.messageContainer.global .playerLocation, .messageContainer.party .playerLocation'
					);
					...
					locMessages.forEach((locMessage) => {
						...
						if (gameId === '2kki' && !localizedMapLocations?.hasOwnProperty(mapId)) {
							...
							if (
								typeof locationCache !== 'undefined' &&
								locationCache?.hasOwnProperty(locationKey) &&
								Array.isArray(locationCache[locationKey])
							) {
								...
								const locationsText = getLocalized2kkiLocations(locations, '&nbsp;|&nbsp;');
								{
									...
									populateMessageNodes(
										parseMessageTextForMarkdown(locationsText),
										messageContents,
										false
									);
								}
							...
							const locationsText = getLocalizedMapLocations(
								gameId,
								mapId,
								prevMapId,
								x,
			...
			if (!translationComplete)
				document.getElementById('translationLink').href =
					`https://github.com/ynoproject/forest-orb/edit/master/lang/${globalConfig.lang}.json`;
			...
			if (isInitial) {
				...
				noGameLocInstruction.classList.toggle(
					'hidden',
					!document.querySelector(`#lang option[value='${globalConfig.lang}']`).dataset.noGameLoc
				);
			}
...
function fetchAndInitLocalizedMapLocations(lang, game) {
	...
	return fetchNewest(`locations/${game}/${fileName}.json`)
		...
		.then((jsonResponse) => {
			...
			Object.keys(gameMapLocations[game]).forEach(function (mapId) {
				...
				if (mapLocation) {
					...
					if (
						Array.isArray(defaultMapLocation) &&
						Array.isArray(mapLocation) &&
						defaultMapLocation.length === mapLocation.length
					) {
				...
				} else gameLocalizedMapLocations[game][mapId] = defaultMapLocation;
			});
			if (game === gameId) {
				localizedLocationUrlRoot = gameLocalizedLocationUrlRoots[game];
				localizedMapLocations = gameLocalizedMapLocations[game];
		...
		.catch((_err) => {
			// Assume map location localizations for this language don't exist
			gameLocalizedLocationUrlRoots[game] = gameLocationUrlRoots[game];
			gameLocalizedMapLocations[game] = gameMapLocations[game];
			if (game === gameId) {
...
function initLocalizedLocations(game) {
	if (!gameMapLocations[game] || !gameLocalizedMapLocations[game]) return;
	...
	for (let mapId in gameMapLocations[game]) {
		...
		if (locations.hasTitle())
			...
			locations.forEach((location, i) =>
				trySetLocalizedLocation(location, gameLocalizedMapLocations[game][mapId][i])
			);
		else {
			for (let key of Object.keys(locations)) {
				const locationsInner = gameMapLocations[game][mapId][key];
				if (!locationsInner) continue;
				if (locationsInner.hasTitle())
					trySetLocalizedLocation(locationsInner, gameLocalizedMapLocations[game][mapId][key]);
				else
					locationsInner.forEach((location, i) =>
						trySetLocalizedLocation(location, gameLocalizedMapLocations[game][mapId][key][i])
					);
			}
		}
	}
...
function getMapLocationsArray(
	...
	if (mapId in mapLocations) {
		...
		if ('else' in locations) {
			// Else case
			if (locations.else.hasTitle()) return [locations.else];
	...
	if (
		...
		const coordLocation = locations.find(
			(l) =>
				...
				((l.coords.x1 === -1 && l.coords.x2 === -1) || (l.coords.x1 <= x && l.coords.x2 >= x)) &&
				((l.coords.y1 === -1 && l.coords.y2 === -1) || (l.coords.y1 <= y && l.coords.y2 >= y))
		);
...
function massageMapLocations(mapLocations, locationUrlTitles) {
	if (Array.isArray(mapLocations)) {
		...
		for (let mapId of Object.keys(mapLocations)) {
			...
			} else massageMapLocations(mapLocation);
		}
	}
}
...
function queryAndSetWikiMaps(locations: unknown[]) {
	...
	Promise.all(
		massagedLocationNames.map((l) =>
			wikiApiFetch('maps', `location=${l}`).then((response) => {
				if (Array.isArray(response)) {
					for (let map of response) maps.push({ url: map.path, label: map.caption });
				}
			})
		)
	).then(() =>
...
function setMaps(maps, locationNames, cacheMaps, saveMaps = false) {
	...
	if (maps && maps.length) {
		for (let map of maps) mapControls.appendChild(getMapButton(map.url, map.label));
...
const badgeHintSe = isBrowser && new Audio('./audio/badge_hint.wav');
let lastMatchedBadgeIdResults;

function updateBadgeHint(locationNames: string[]) {
	if (!badgeCache || !Array.isArray(badgeCache)) return;
	...
	for (const b of badgeCache) {
		if (b.game !== gameId || b.unlocked || b.hidden) continue;
		...
		if (locationNames.includes(title)) {
			if (!matchedLocationName) matchedLocationName = title;
	...
	if (matchedLocationName !== null) {
		if (globalConfig.playBadgeHintSound) badgeHintSe.play();
...
function createInputElement(type, id, placeholder, onUpdate, checked = false, ...classes) {
	...
	input.type = type;
	input.id = id;
	if (classes.length) input.className = classes.join(' ');
...
function openWikiLink(url, useDefault, asImage = false) {
	...
	const currentMode = wikiLinkModeSelect
		? parseInt(/** @type {HTMLSelectElement} */ wikiLinkModeSelect.value)
		: globalConfig.wikiLinkMode;
				...
				const localValue = value
					? Object.fromEntries(
							Object.keys(value)
								.filter(
									(k) =>
										value[k].hasOwnProperty('time') &&
										(currentTime - value[k].time) / (1000 * 3600 * 24) < 7
								)
								.map((k) => [k, value[k].value])
						)
					: {};
				if (k === CACHE_TYPE.location) locationCache = localValue;
				else if (k === CACHE_TYPE.map) mapCache = localValue;
				else if (k === CACHE_TYPE.locationColor) locationColorCache = localValue;
				if (value) cache[k] = value;
			};
		}
	};
}
...
function clearCache(cacheType, el: HTMLElement | null) {
	...
	switch (cacheType) {
		case CACHE_TYPE.location:
			locationCache = {};
			break;
		case CACHE_TYPE.locationColor:
			locationColorCache = {};
			break;
		case CACHE_TYPE.map:
			mapCache = {};
		...
		if (!globalConfig.warningsReviewed) {
			...
			if (warningsModal) {
				const observer = new MutationObserver((mutations) => {
					if (
						!warningsModal.classList.contains('hidden') &&
						!warningsModal.classList.contains('fadeIn') &&
