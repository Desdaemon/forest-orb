...
import { addTooltip, updateConfig, wikiApiFetch, hasTouchscreen } from "./init";
import { addSessionCommandHandler, initSessionWs } from "./session";
import { gameId, isBrowser } from "$lib";
import { getDefaultUiTheme, populateUiThemes, setSystemName, systemName } from "./system";
    ...
    .then(jsonResponse => {
      if (jsonResponse.uuid) {
        ...
        if (jsonResponse.name)
          playerName.set(jsonResponse.name);
...
export function openModal(modalId: string, theme?: string, lastModalId?: string, modalData?: object) {
  ...
  setModalUiTheme(modalId, theme || (config.uiTheme === 'auto' ? get(systemName) : config.uiTheme));
