import { gameId, isBrowser, extractCanvas } from '$lib';
import type { ClassValue } from 'svelte/elements';
import { addOrUpdatePlayerListEntry, playerData } from './playerlist.svelte';
import { getConnStatus } from './session.svelte';
import { config } from './config.svelte';

let systemName: string | undefined = $state();
...
export const allGameUiThemes = {
	...
	cold: ['system_F_1', 'system_F', 'system_C', 'system_R', 'system_FC', 'system_FC_1'],
	unconscious: [
		'system',
		'system2',
		'maple-penmenu',
...
function onUpdateSystemGraphic(name) {
	if (gameUiThemes.indexOf(name.replace(/'|\s$/g, '')) > -1) {
		...
		if (lastAutoButton) lastAutoButton.remove();
...
export function getDefaultUiTheme(themeGameId?: string) {
	if (!themeGameId || !allGameUiThemes.hasOwnProperty(themeGameId)) themeGameId = gameId;
	return allGameUiThemes[themeGameId][0];
}

export function setUiTheme(value: string, isInit = false) {
	if (!isBrowser) return;

	const isAuto = value === 'auto';
	const uiTheme = isAuto ? systemName || getDefaultUiTheme() : value;
	if (gameUiThemes.indexOf(uiTheme) === -1) return;
	...
	if (lastSelectedThemeContainer) lastSelectedThemeContainer.classList.remove('selected');
	if (newSelectedTheme) newSelectedTheme.parentElement.classList.add('selected');
	const useFullBg = gameFullBgUiThemes.indexOf(uiTheme) > -1;
	const containers = document.querySelectorAll('.container');
	for (let container of containers) container.classList.toggle('fullBg', useFullBg);
	const themedContainers = document.querySelectorAll('.listEntry, .toast');
	for (let themedContainer of themedContainers) updateThemedContainer(themedContainer);
	...
	if (isInit) setPartyTheme(uiTheme);
	else {
...
function setPartyTheme(value) {
	...
	const newSelectedTheme = document.querySelector(`.uiTheme[data-ui-theme="${value}"]`);
	if (lastSelectedThemeContainer) lastSelectedThemeContainer.classList.remove('partySelected');
	if (newSelectedTheme) newSelectedTheme.parentElement.classList.add('partySelected');
}
...
function setScheduleTheme(value) {
	...
	const newSelectedTheme = document.querySelector(`.uiTheme[data-ui-theme="${value}"]`);
	if (lastSelectedThemeContainer) lastSelectedThemeContainer.classList.remove('scheduleSelected');
	if (newSelectedTheme) newSelectedTheme.parentElement.classList.add('scheduleSelected');
}

export function setFontStyle(fontStyle, isInit = false) {
	if (!isBrowser) return;
...
function initUiThemeContainerStyles(uiTheme, themeGameId, setTheme, callback) {
	...
	getBaseBgColor(uiTheme, themeGameId, function (color) {
		getFontShadow(uiTheme, themeGameId, function (shadow) {
			...
			if (callback) callback(color, shadow);
		});
	});
}
		...
		const altColorCallback = function (altColors) {
			...
			if (!rootStyle.getPropertyValue(baseColorProp)) {
				...
				rootStyle.setProperty(
					...
					`url('images/ui/${themeGameId}/${uiTheme}/font${fontStyle + 1}.png')`
				);
			}
			...
			if (callback) callback(baseColors, altColors);
		};
...
export function themeClass(
	...
	return [
		themeStylesId,
		{ fullBg: (allGameFullBgUiThemes[themeGameId] as readonly string[]).includes(uiTheme) }
	] as const;
}

{
  ...
  `;
	...
	applyThemeStyles = (el, uiTheme, themeGameId) => {
		if (!el || !uiTheme) return Promise.resolve();
		if (!themeGameId) themeGameId = gameId;
		...
		if (!themeStyles) {
			...
			themeStyles.innerHTML = themeStyleTemplate
				...
				.replace(
					/\{FULL_BG\|(.*?)\}/,
					(allGameFullBgUiThemes[themeGameId] || []).indexOf(uiTheme) > -1 ? '$1' : ''
				);
			task = void document.head.appendChild(themeStyles);
		}
		const applyThemeClass = () => {
			for (let cls of el.classList) {
				if (cls.startsWith('theme_')) {
					if (cls === themeStylesId) continue;
		...
		if (!el.parentElement) {
			// detached element, modify immediately
			applyThemeClass();
			return task;
		} else return Promise.allSettled([fastdom.mutate(applyThemeClass), task]);
	};
}
...
function getUiThemeOption(uiTheme) {
	...
	if (gameFullBgUiThemes.indexOf(uiTheme) > -1) option.classList.add('fullBg');
...
function getFontColors(uiTheme, themeGameId, fontStyle, callback) {
	...
	if (colors) return callback(colors);
	const img = new Image();
	img.onload = () => getFontColorsImgLoaded(img, themeGameId, uiTheme, fontStyle, callback);
	img.src = `images/ui/${themeGameId}/${uiTheme}/font${fontStyle + 1}.png`;
}

async function getFontColorsImgLoaded(img, themeGameId, uiTheme, fontStyle, callback) {
	await img.decode();
	...
	for (let i = 0; i < data.length; i += 4) colors[i / 4] = [data[i], data[i + 1], data[i + 2]];
...
function getFontShadow(uiTheme, themeGameId, callback) {
	...
	if (pixel) return callback(getColorRgb(pixel));
	const img = new Image();
...
function addSystemSvgGradient(systemName, systemGameId, colors, alt) {
	if (!systemGameId) systemGameId = gameId;
...
function getStylePropertyValue(name) {
	...
	if (value && value.startsWith('var(')) return getStylePropertyValue(value.slice(4, -1));
	return value;
}
