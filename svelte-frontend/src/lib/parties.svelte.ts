...
/** The create/update party form, rendered by `<CreatePartyModal />`. */
export const partyForm = $state({
	name: '',
	description: '',
	password: '',
	/** `true` while the party is private, i.e. while `#publicPartyButton` is toggled. */
	private: false,
	showPassword: false
});

/** The private party password prompt, rendered by `<JoinPrivatePartyModal />`. */
export const joinPartyForm = $state({
	password: '',
	showPassword: false,
	failed: false
});

function resetPartyForm(party?) {
	partyForm.name = party?.name || '';
	partyForm.description = party?.description || '';
	partyForm.password = '';
	partyForm.private = party ? !party.public : false;
	partyForm.showPassword = false;
}

/** Bound to `#createPartyButton` in `<ChatboxContainer />`. */
...
/** Bound to `#partyThemeButton` in `<CreatePartyModal />`. */
export function openPartyThemeModal() {
	openModal('uiThemesModal', document.getElementById('partyTheme').value, 'createPartyModal');
}

/** Bound to `#createPartyForm` in `<CreatePartyModal />`. */
export function submitCreatePartyForm(ev) {
...
/** Bound to `#disbandPartyButton` in `<ChatboxContainer />`. */
...
/** Bound to `#joinPrivatePartyForm` in `<JoinPrivatePartyModal />`. */
export function submitJoinPrivatePartyForm(ev) {
	const formData = new FormData(ev.currentTarget);
	const partyId = document.getElementById('joinPrivatePartyModal').dataset.partyId;
	closeModal();
	apiFetch(`party?command=join&partyId=${partyId}&${new URLSearchParams(formData).toString()}`)
		.then((response) => {
			if (!response.ok) {
				if (response.status === 401) {
					joinPartyForm.failed = true;
...
function getPartyName(party, includeLock, asHtml) {
	...
	let partyName =
		(isPartyObj ? party.name : party) || localizedMessages.parties.defaultPartyName.replace('{OWNER}', ownerName);
	...
	if (asHtml) {
		...
		if (includeLock && !party.public) {
			...
			if (party.systemName) {
				...
				if (gameUiThemes.indexOf(partySystemName) === -1) partySystemName = getDefaultUiTheme();
				const parsedPartySystemName = partySystemName.replace(/ /g, '_');
				partyLockIcon
					.querySelector('path')
					.setAttribute('style', `fill: var(--svg-base-gradient-${parsedPartySystemName});`);
			}
	...
	} else if (includeLock && !party.public) partyName = `🔒 ${partyName}`;

	return partyName;
}

function getPartyMemberName(party, partyMember, includeRoles, asHtml) {
	if (typeof partyMember === 'string') partyMember = party ? party.members.find((m) => m.uuid === partyMember) : null;
	...
	if (includeRoles && party && party.ownerUuid === partyMember?.uuid) partyMemberName += roleEmojis.partyOwner;
	return partyMemberName;
}
...
function setJoinedPartyId(partyId) {
	...
	if (partyId)
		updateJoinedParty(() =>
			content.classList.toggle('partyOwner', playerData?.uuid === joinedPartyCache.ownerUuid)
		);
	else {
		...
		if (gameChatModeIndex === 2) cycleGameChatMode();
	}
}
...
function transferJoinedPartyOwner(playerUuid) {
	...
	if (joinedPartyCache && joinedPartyCache.ownerUuid === playerData?.uuid) {
		showConfirmModal(
			localizedMessages.parties.confirmTransferPartyOwner.replace('{PLAYER}', targetPlayerName),
			() => {
...
function updatePartyList() {
	apiFetch(`party?command=list`)
		...
		.then((data) => {
			...
			const removedPartyIds = Object.keys(partyCache)
				.map((p) => parseInt(p))
				.filter((p) => partyIds.indexOf(p) === -1);
			...
			if (data.length) {
				...
				for (let party of data) {
					...
					if (isInParty) {
						if (partyCache[party.id]) {
							...
							const newMemberUuids = memberUuids.filter(
								(uuid) => oldMemberUuids.indexOf(uuid) === -1 && uuid !== playerData.uuid
							);
							const removedMemberUuids = oldMemberUuids.filter(
								(uuid) => memberUuids.indexOf(uuid) === -1 && uuid !== playerData.uuid
							);
							for (let uuid of newMemberUuids) showPartyToastMessage('playerJoin', 'join', party, uuid);
				...
				const entries = [].slice.call(partyListEntries).sort(function (a, b) {
					...
					if (partyA.id == joinedPartyId) return -1;
					if (partyB.id == joinedPartyId) return 1;
					if (partyA.public !== partyB.public) return partyA.public ? -1 : 1;
			...
			if (joinedPartyId) {
				...
				} else joinedPartyLabel.innerHTML = joinedPartyLabelHtml;
			...
			if (activePartyModal) initOrUpdatePartyModal(activePartyModal.dataset.partyId);
		})
		.catch((err) => console.error(err));
}
...
function onUpdateJoinedParty(party) {
	...
	if (party.systemName !== joinedPartyUiTheme) setPartyUiTheme(party.systemName);
	...
	for (let playerUuid of removedPlayerUuids) removePlayerListEntry(partyPlayerList, playerUuid);
	...
	for (let member of party.members) {
		...
		if (oldMember) {
			...
			if (member.online !== oldMember.online) {
				...
				} else joinedPartyPendingOfflineMemberUuids.push(uuid);
		...
		if (member.badge === 'null') member.badge = null;
...
function addOrUpdatePartyListEntry(party) {
	...
	const partyListEntrySprite = partyListEntry
		? partyListEntry.querySelector('.partyListEntrySprite')
		: document.createElement('img');
	const nameText = partyListEntry ? partyListEntry.querySelector('.nameText') : document.createElement('span');
	const memberCount = partyListEntry
		? partyListEntry.querySelector('.partyListEntryMemberCount')
		: document.createElement('div');
	const memberCountText = partyListEntry
		? memberCount.querySelector('.partyListEntryMemberCountText')
		: document.createElement('span');
	const partyMemberSpritesContainer = partyListEntry
		? partyListEntry.querySelector('.partyMemberSpritesContainer')
		: document.createElement('div');
	const partyListEntryActionContainer = partyListEntry
		? partyListEntry.querySelector('.partyListEntryActionContainer')
		: document.createElement('div');
	...
	if (!partyListEntry) {
		...
		addTooltip(
			viewDetailsAction,
			getMassagedLabel(localizedMessages.parties.actions.viewPartyDetails, true),
			true,
			true
		);
	...
	} else partyMemberSpritesContainer.innerHTML = '';
	...
	if (!joinedPartyId || isInParty) {
		...
		if (!joinLeaveAction || !joinLeaveAction.classList.contains(actionClass)) {
			if (joinLeaveAction) joinLeaveAction.remove();
			...
			joinLeaveAction.onclick = isInParty
				...
				: party.public || playerData?.rank
					...
					: function () {
							joinPartyForm.password = '';
							joinPartyForm.showPassword = false;
							joinPartyForm.failed = false;
							openModal('joinPrivatePartyModal', party.systemName, null, { partyId: party.id });
						};
			addTooltip(
				joinLeaveAction,
				getMassagedLabel(
					localizedMessages.parties.actions[
						`${isInParty ? 'leave' : party.public || playerData?.rank ? 'join' : 'joinPrivate'}Party`
					],
					true
				),
				true,
	...
	} else if (joinLeaveAction) joinLeaveAction.remove();

	partyListEntry.classList.toggle('joinedParty', joinedPartyId);

	if (party.systemName) {
		let systemName = party.systemName.replace(/'/g, '');
		if (gameUiThemes.indexOf(systemName) === -1) systemName = getDefaultUiTheme();
	...
	if (isInParty) {
		...
		for (let uuid of removedMemberUuids) removePartyListEntry(uuid);
	}
	...
	for (let m = 0; m < party.members.length; m++) {
		...
		const playerSpriteCacheEntry = (playerSpriteCache[member.uuid] = {
			sprite: member.spriteName,
			idx: member.spriteIndex
		});
...
function initOrUpdatePartyModal(partyId) {
	...
	if (!party.public) modalTitle.prepend(getSvgIcon('locked', true));
	...
	if (lastPartyId) {
		if (partyId == lastPartyId) {
			const oldOnlinePlayerUuids = Array.from(partyModalOnlinePlayerList.querySelectorAll('.listEntry')).map(
				(e) => e.dataset.uuid
			);
			const oldOfflinePlayerUuids = Array.from(partyModalOfflinePlayerList.querySelectorAll('.listEntry')).map(
				(e) => e.dataset.uuid
			);

			const removedOnlinePlayerUuids = oldOnlinePlayerUuids.filter(
				(uuid) => !party.members.find((m) => m.uuid === uuid && m.online)
			);
			const removedOfflinePlayerUuids = oldOfflinePlayerUuids.filter(
				(uuid) => !party.members.find((m) => m.uuid === uuid && !m.online)
			);
		...
		apiFetch(`party?command=description&partyId=${partyId}`)
			...
			.then((description) => {
				...
				if (partyModal.dataset.partyId == partyId) {
					...
					if (description) partyDescriptionContainer.classList.remove('hidden');
				}
...
function showPartyToastMessage(key, icon, party, playerUuid) {
	if (!notificationConfig.parties.all || !notificationConfig.parties[key]) return;
	let message = getMassagedLabel(localizedMessages.toast.parties[key], true).replace('{PARTY}', getPartyName(party));
	if (playerUuid) message = message.replace('{PLAYER}', getPartyMemberName(party, playerUuid, true, true));
