import { isBrowser, hasTouchscreen } from '$lib';
import { globalConfig } from './config.svelte';
import { onResize } from './play.svelte';

const preventNativeKeys = ['ArrowUp', 'ArrowDown', 'ArrowRight', 'ArrowLeft', ' ', 'F12'];
const keys = new Map();
const keysDown = new Map();
/**
 * The game canvas and its container. Resolved lazily by the initializers below, which are
 * called by `<GameCanvas />` once the elements are actually in the document.
 */
let canvas;
let gameContainer;
...
/**
 * Makes the EasyRPG player embeddable and recovers from lost WebGL contexts.
 * Called on mount from `<GameCanvas />`.
 */
...
/** Toggles fullscreen on the layout. Bound to `#controls-fullscreen` in `+page.svelte`. */
...
/** Whether this browser can put the layout into fullscreen at all. */
export function isFullscreenSupported() {
	// assume support while rendering on the server, so the button doesn't flash hidden
	if (!isBrowser) return true;
	const layout = document.getElementById('layout');
	return !!(layout?.requestFullscreen || layout?.webkitRequestFullscreen);
}
 ...
 * @param keyCode Key code to simulate (deprecated)
 */
export function simulateKeyboardInput(key: string, keyCode: number) {
...
function scanGamePads() {
	...
	for (let i = 0; i < pads.length; i++) {
		if (pads[i]) {
			if (pads[i].index in gamepads) gamepads[pads[i].index] = pads[i];
			else addGamepad(pads[i]);
		}
	}
}

/**
 * Starts tracking connected gamepads. Called on mount from `<GameCanvas />`.
 * Browsers without gamepad events are polled instead.
 */
...
function updateTouchControlsVisibility() {
	if (hasTouchscreen && !Object.keys(gamepads).length) {
		...
		for (const elem of document.querySelectorAll('#dpad, #apad')) elem.style.display = 'none'; // Hide the touch controls
	}
}

/**
 * Binds the touch controls (or, on non-touch devices, the canvas keyboard shortcuts).
 * Called on mount from `<GameCanvas />`, once the controls exist in the document.
 */
	...
	if (hasTouchscreen) {
		...
		canvasContainer.addEventListener('touchstart', (ev) => {
			...
			switch (availableControlType) {
				case 'joystick':
					radius = joystickCircle.getBoundingClientRect().width / 2;
					break;
				case 'dpad':
					radius = dpadCircle.getBoundingClientRect().width / 2;
					break;
				case 'default':
				default:
					return;
			...
			requestAnimationFrame(function driveControls() {
				...
				if (anchor && compass) {
					...
					if (Math.sqrt(dx * dx + dy * dy) >= deadzone && (direction = directions[lastDirection])) {
						...
						switch (availableControlType) {
							case 'dpad':
								for (const { button } of Object.values(directions)) button.classList.remove('active');
		...
		canvas.addEventListener('keydown', (event) => {
			if (preventNativeKeys.includes(event.key)) event.preventDefault();
				...
				let input;
				if (chatInput.offsetWidth) input = chatInput;
				else {
					const nameInput = document.getElementById('nameInput');
					if (nameInput.offsetWidth) input = nameInput;
				}
				if (input) window.setTimeout(() => input.focus(), 0);
			} else {
				const keyLc = event.key.toLowerCase();
				if (
					globalConfig.gameChat &&
					(keyLc === 't' ||
						keyLc === 'm' ||
