...
if ('serviceWorker' in navigator) {
	...
	navigator.serviceWorker.ready.then(async (registration) => {
		...
		if (
			permissions !== 'denied' &&
			!hasSubscription &&
			!globalConfig.pushNotificationToastDismissed &&
			notificationConfig.system.all &&
			notificationConfig.system.pushNotifications
		) {
		...
		switch (_type) {
			case 'toast':
				...
				if (
					metadata &&
					!(
						notificationConfig[metadata.category]?.all &&
						notificationConfig[metadata.category]?.[metadata.type]
					)
				)
					break;
