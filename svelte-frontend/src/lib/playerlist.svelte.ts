import { gameDefaultSprite, isBrowser, ynoGameId } from '$lib';
import { addSessionCommandHandler } from './session.svelte';
import { allGameUiThemes } from './system.svelte';
...
export const playerData = $state(null);
let playerUuids = {};
...
function getPlayerName(player, includeMarkers, includeBadge, asHtml) {
	...
	if (unnamed) playerName = localizedMessages.playerList.unnamed;
	...
	if (asHtml && isPlayerObj) {
		...
		if (player.rank) {
			...
			addTooltip(
				rankIcon,
				getMassagedLabel(localizedMessages.roles[rank === 1 ? 'mod' : 'dev'], true),
				true,
				true
			);
		...
		if (
			playerFriendsCache.find(
				(pf) => pf.accepted && (isPlayerObj ? pf.uuid === player.uuid : pf.name === playerName)
			)
		) {
			...
			addTooltip(
				friendIcon,
				document.createTextNode(getMassagedLabel(localizedMessages.friends.friend)),
				true,
				true
			);
		...
		if (badge) {
			...
			badgeOverlay2El =
				badge?.overlayType & BadgeOverlayType.DUAL ? document.createElement('div') : null;
			...
			if (badgeOverlayEl) {
				...
				const badgeMaskUrl =
					badge.overlayType & BadgeOverlayType.MASK
						? badgeUrl.replace(
								'.',
								badge.overlayType & BadgeOverlayType.DUAL ? '_mask_fg.' : '_mask.'
							)
						: badgeUrl;

				badgeOverlayEl.setAttribute(
					'style',
		...
		if (player.systemName) {
			...
			if (unnamed || gameUiThemes.indexOf(systemName) === -1) systemName = getDefaultUiTheme();
			const parsedSystemName = systemName.replace(/ /g, '_');
			applyThemeStyles(nameTextContainer, parsedSystemName);
			if (badgeOverlayEl) {
				if (badgeOverlay2El) {
					badgeOverlayEl.style.background =
						badge.overlayType & BadgeOverlayType.GRADIENT
					...
					if (
						...
						badgeOverlay2El.style.background =
							badge.overlayType & BadgeOverlayType.GRADIENT
								? `var(--alt-gradient-${parsedSystemName})`
								: `rgb(var(--alt-color-${parsedSystemName}))`;
					} else badgeOverlay2El.style.background = `rgb(var(--base-bg-color-${parsedSystemName}))`;
				} else badgeOverlayEl.style.background = `var(--base-gradient-${parsedSystemName})`;
			}
		}
	...
	if (includeMarkers && isPlayerObj) {
		if (player.rank) playerName += roleEmojis[player.rank === 1 ? 'mod' : 'dev'];
		if (player.muted) playerName += roleEmojis['muted'];
		if (!asHtml && !player.account) playerName = `<${playerName}>`;
	}

	return playerName;
}

export function addOrUpdatePlayerListEntry(playerList, player, showLocation, sortEntries) {
	...
	if (typeof player.spriteIndex === 'number' && player.spriteIndex >= 0) {
		...
		if (
			isObjectSprite &&
			gameDefaultSprite[spriteGameId] &&
			typeof gameDefaultSprite[spriteGameId].idx === 'number'
		) {
	...
	if (spriteName) {
		getSpriteProfileImg(spriteName, spriteIndex, undefined, undefined, spriteGameId).then(
			(spriteImg) => {
				if (!spriteImg) return;
				if (!playerList) playerList = document.getElementById('playerList');
				const sprite = playerList.querySelector(
					`.playerListEntry[data-uuid="${uuid}"] img.playerListEntrySprite`
				);
				if (sprite && sprite.src !== spriteImg) sprite.src = spriteImg;
			}
		);
	} else {
		...
		if (playerSpriteCacheEntry) {
			...
			getSpriteProfileImg(entry.sprite, entry.idx, undefined, undefined, spriteGameId).then(
				(spriteImg) => {
					if (!spriteImg) return;
					if (!playerList) playerList = document.getElementById('playerList');
					const sprite = playerList.querySelector(
			...
			if (uuid === defaultUuid) updatePlayerSprite(entry.sprite, entry.idx, spriteGameId);
		} else {
			const isObjectSprite = typeof gameDefaultSprite === 'object';
			let defaultSprite = '';
			let defaultIdx = 0;
			if (isObjectSprite && gameDefaultSprite[spriteGameId]) {
				...
				defaultIdx =
					typeof gameDefaultSprite[spriteGameId].idx === 'number'
						? gameDefaultSprite[spriteGameId].idx
						: 0;
			} else if (!isObjectSprite) {
			...
			if (defaultSprite) {
				getSpriteProfileImg(defaultSprite, defaultIdx, undefined, undefined, spriteGameId).then(
					(spriteImg) => {
						if (!spriteImg) return;
						if (!playerList) playerList = document.getElementById('playerList');
						const sprite = playerList.querySelector(
	...
	const nameText = playerListEntry
		? playerListEntry.querySelector('.nameText')
		: document.createElement('span');
	const playerListEntrySprite = playerListEntry
		? playerListEntry.querySelector('.playerListEntrySprite')
		: document.createElement('img');
	const playerListEntryMedals = playerListEntry
		? playerListEntry.querySelector('.playerListEntryMedals')
		: document.createElement('div');
	const playerListEntryBadge = playerListEntry
		? playerListEntry.querySelector('.playerListEntryBadge')
		: document.createElement('div');
	const playerListEntryBadgeOverlay = playerListEntry
		? playerListEntryBadge.querySelector('.playerListEntryBadgeOverlay')
		: document.createElement('div');
	const playerListEntryBadgeOverlay2 = playerListEntry
		? playerListEntryBadge.querySelector('.playerListEntryBadgeOverlay2')
		: document.createElement('div');
	const playerListEntryActionContainer = playerListEntry
		? playerListEntry.querySelector('.playerListEntryActionContainer')
		: document.createElement('div');
	...
	let acceptFriendAction = playerListEntry
		? playerListEntry.querySelector('.acceptFriendAction')
		: null;
	let rejectCancelFriendAction = playerListEntry
		? playerListEntry.querySelector('.rejectCancelFriendAction')
		: null;
	...
	let transferPartyOwnerAction = playerListEntry
		? playerListEntry.querySelector('.transferPartyOwnerAction')
		: null;
	...
	if (!playerListEntry) {
		...
		playerListEntryBadgeOverlay2.classList.add(
			'playerListEntryBadgeOverlay2',
			'badgeOverlay',
			'badgeOverlay2'
		);
		...
		playerListEntryActionContainer.classList.add(
			'playerListEntryActionContainer',
		...
		const shouldScroll =
			playerList.scrollHeight > playerList.clientHeight &&
			Math.abs(playerList.scrollHeight - playerList.scrollTop - playerList.clientHeight) <= 20;

		playerList.appendChild(playerListEntry);

		if (shouldScroll) playerList.scrollTop = playerList.scrollHeight;
	}

	if (name || !nameText.innerText || playerList.id !== 'playerList') {
		const playerNameText = getPlayerName(player);
		nameText.innerText = playerNameText;
		if (player.account) {
			const nameMarkers = nameText.parentElement.querySelectorAll('.nameMarker');
			for (let nameMarker of nameMarkers) nameMarker.remove();
		}

		if (name) delete playerListEntry.dataset.unnamed;
		else playerListEntry.dataset.unnamed = 'unnamed';
	...
	if (spriteName) {
		getSpriteProfileImg(spriteName, spriteIndex, undefined, undefined, playerGameId).then(
			(spriteImg) => {
				const sprite = playerList.querySelector(
					`.playerListEntry[data-uuid="${uuid}"] img.playerListEntrySprite`
				);
				if (spriteImg && sprite && sprite.src !== spriteImg) sprite.src = spriteImg;
			}
		);
	} else {
		...
		if (playerSpriteCacheEntry) {
			const entry = playerSpriteCacheEntry;
			getSpriteProfileImg(entry.sprite, entry.idx, undefined, undefined, playerGameId).then(
				(spriteImg) => {
					const sprite = playerList.querySelector(
						`.playerListEntry[data-uuid="${uuid}"] img.playerListEntrySprite`
					);
			...
			if (uuid === defaultUuid) updatePlayerSprite(entry.sprite, entry.idx, playerGameId);
		} else {
			const defaultSpriteObj = getDefaultSprite();
			getSpriteProfileImg(
				defaultSpriteObj.sprite,
				defaultSpriteObj.idx,
				undefined,
				undefined,
				playerGameId
			).then((spriteImg) => {
	...
	if (showBadgeOverlay) {
		...
		playerListEntryBadgeOverlay.classList.toggle(
			'badgeOverlayMultiply',
			badge.overlayType & BadgeOverlayType.MULTIPLY
		);
		playerListEntryBadgeOverlay.setAttribute(
			'style',
			`-webkit-mask-image: url('${badgeMaskUrl}'); mask-image: url('${badgeMaskUrl}');`
		);
		...
		if (showBadgeOverlay2) {
			...
			playerListEntryBadgeOverlay2.classList.toggle(
				'badgeOverlayMultiply',
				badge.overlayType & BadgeOverlayType.MULTIPLY
			);
			playerListEntryBadgeOverlay2.setAttribute(
				'style',
				`-webkit-mask-image: url('${badgeMask2Url}'); mask-image: url('${badgeMask2Url}');`
			);
		}
	...
	if (showBadge) {
		if (localizedBadges) {
			...
			if (badgeGame) {
				playerListEntryBadge._badgeTippy = addOrUpdateTooltip(
					playerListEntryBadge,
					document.createTextNode(getMassagedLabel(localizedBadges[badgeGame][player.badge].name)),
					true,
					true,
					false,
					null,
					playerListEntryBadge._badgeTippy
				);
				if (!badge || badge.hidden)
					playerListEntryBadge._badgeTippy.popper
						.querySelector('.tooltipContent')
						.classList.add('altText');
			}
		}
		if (player.name) {
			// Doesn't support x/y so location color badges may be incorrect for maps with multiple locations
			addOrUpdatePlayerBadgeGalleryTooltip(
				playerListEntryBadge,
				player.name,
				(player.systemName || getDefaultUiTheme(playerGameId)).replace(/'/g, '')
			);
			playerListEntryBadge.classList.toggle('badgeButton', player.name);
		}
	}

	if (acceptFriendAction) acceptFriendAction.remove();
	if (rejectCancelFriendAction) rejectCancelFriendAction.remove();

	if (partyOwnerIcon) partyOwnerIcon.remove();
	if (partyKickAction) partyKickAction.remove();
	if (transferPartyOwnerAction) transferPartyOwnerAction.remove();

	if (playerList.id === 'friendsPlayerList') {
		const playerFriend = playerFriendsCache.find((pf) => pf.uuid === uuid);
		if (showMedals) showMedals = playerFriend?.accepted;
		if (playerFriend && !playerFriend.accepted) {
			if (playerFriend.incoming) {
				...
				addTooltip(
					acceptFriendAction,
					getMassagedLabel(localizedMessages.playerList.actions.approveFriend, true),
					true,
					true
				);
				...
				apiFetch(`removeplayerfriend?uuid=${uuid}`)
					...
					.then((_) => {
						...
						if (cachedPlayerFriend) {
							...
							showFriendsToastMessage(
								playerFriend.incoming ? 'reject' : 'cancel',
								'deny',
								cachedPlayerFriend
							);
						}
			...
			addTooltip(
				rejectCancelFriendAction,
				getMassagedLabel(
					localizedMessages.playerList.actions[
						playerFriend.incoming ? 'rejectFriend' : 'cancelFriend'
					],
					true
				),
				true,
		...
		let party;
		if (playerList.id === 'partyPlayerList') party = joinedPartyCache;
		...
		if (party) {
			if (
				uuid === party.ownerUuid ||
				(uuid === defaultUuid && playerData?.uuid === party.ownerUuid)
			) {
				partyOwnerIcon = getSvgIcon('partyOwner', true);
				addTooltip(
					partyOwnerIcon,
					getMassagedLabel(localizedMessages.parties.partyOwner, true),
					true,
					true
				);
				if (party.systemName) {
					...
					partyOwnerIcon
						...
						.setAttribute(
							'style',
							`fill: var(--svg-base-gradient-${parsedPartySystemName}); filter: var(--svg-shadow-${parsedPartySystemName});`
						);
				}
				...
				addTooltip(
					partyKickAction,
					getMassagedLabel(localizedMessages.playerList.actions.partyKick, true),
					true,
					true
				);
				...
				addTooltip(
					transferPartyOwnerAction,
					getMassagedLabel(localizedMessages.playerList.actions.transferPartyOwner, true),
					true,
					true
				);
	...
	if (
		playerListEntry.dataset.unnamed ||
		!(themeGameId in allGameUiThemes) ||
		allGameUiThemes[themeGameId].indexOf(systemName) === -1
	)
		systemName = getDefaultUiTheme(themeGameId);
	...
	initUiThemeContainerStyles(systemName, themeGameId, false, () => {
		initUiThemeFontStyles(systemName, themeGameId, 0, false, () => {
			...
			if (showBadgeOverlay) {
				playerListEntryBadgeOverlay.style.background =
					badge.overlayType & BadgeOverlayType.GRADIENT
						? `var(--base-gradient-${gameParsedSystemName})`
						: `rgb(var(--base-color-${gameParsedSystemName}))`;
				if (showBadgeOverlay2) {
					if (
						getStylePropertyValue(`--base-color-${gameParsedSystemName}`) !==
						getStylePropertyValue(`--alt-color-${gameParsedSystemName}`)
					) {
						playerListEntryBadgeOverlay2.style.background =
							badge.overlayType & BadgeOverlayType.GRADIENT
	...
	if (sortEntries) sortPlayerListEntries(playerList);

	if (playerList.id === 'playerList') updateMapPlayerCount(playerList.childElementCount);
...
function addOrUpdatePlayerListEntryLocation(locationVisible, player, entry) {
	...
	playerLocation.classList.toggle(
		'hidden',
		(!locationVisible || !shouldDisplayLocation) && !showLastOnline
	);

	if (locationVisible && player.online && shouldDisplayLocation) {
		if (!(playerGameId in allGameUiThemes)) playerGameId = gameId;
		const parsedSystemName = player.systemName
			? (allGameUiThemes[playerGameId].indexOf(player.systemName) > -1
					? player.systemName
					: getDefaultUiTheme(playerGameId)
				).replace(/ /g, '_')
			: null;
		...
		if (playerGameId === gameId) {
			if (gameId === '2kki' && !localizedMapLocations?.hasOwnProperty(player.mapId)) {
				const prevLocations =
					player.prevLocations && player.prevMapId !== '0000'
						? decodeURIComponent(window.atob(player.prevLocations))
								.split('|')
								.map((l) => {
									return { title: l };
								})
						: null;
				set2kkiGlobalChatMessageLocation(
					playerLocation,
					player.mapId,
					player.prevMapId,
					prevLocations
				...
				const locationsHtml = getLocalizedMapLocationsHtml(
					gameId,
					player.mapId,
					player.prevMapId,
					player.x,
		...
		if (parseInt(lastActive) < 5000)
			infoLabel.innerHTML = getMassagedLabel(localizedMessages.lastOnline.template).replace(
				'{INTERVAL}',
				lastActive
			);
		else infoLabel.innerHTML = getMassagedLabel(localizedMessages.lastOnline.longTime);
		{
...
function updatePlayerListEntryHeader(playerList, key, categoryId) {
	...
	header.innerHTML = getMassagedLabel(
		localizedMessages[key].categories[categoryId].replace('{COUNT}', elements.length),
		true
	);
...
function updatePlayerListEntrySprite(playerList, sprite, idx, uuid, gameId) {
	...
	const playerListEntrySprite = playerList.querySelector(
		`.playerListEntry[data-uuid="${uuid}"] img.playerListEntrySprite`
	);
	...
	getSpriteProfileImg(sprite, idx, undefined, undefined, gameId).then((spriteImg) => {
		if (
			...
			playerSpriteCache[cacheKey].sprite === sprite &&
			playerSpriteCache[cacheKey].idx === idx
		)
			playerListEntrySprite.src = spriteImg;
	});

	if (uuid === defaultUuid) updatePlayerSprite(sprite, idx, gameId);
}
...
function removePlayerListEntry(playerList, uuid) {
	...
	if (playerListEntry) playerListEntry.remove();
...
function clearPlayerList(playerList) {
	...
	if (playerList.id === 'playerList') updateMapPlayerCount(0);
}
		...
		switch (playerListId) {
			...
			case 'partyPlayerList':
				...
				const baseFunc = (a, b) => {
					...
					if (playerA?.rank !== playerB?.rank) return playerA?.rank < playerB?.rank ? 1 : -1;
					if (a.dataset.name && b.dataset.name)
						return collator.compare(a.dataset.name, b.dataset.name);
					if (a.dataset.unnamed) {
						if (b.dataset.unnamed) return a.dataset.uuid >= b.dataset.uuid ? 1 : -1;
						return 1;
					}
					if (b.dataset.unnamed) return -1;
					if (playerA?.account != playerB?.account) return playerA?.account ? -1 : 1;
					return 0;
				};
				return playerListId === 'playerList'
					? (a, b) => {
							if (a.dataset.uuid === defaultUuid) return -1;
							if (b.dataset.uuid === defaultUuid) return 1;
							return baseFunc(a, b);
						}
					: (a, b) => {
							...
							} else if (memberA) return -1;
							else if (memberB) return 1;
							if (a.dataset.uuid === playerData?.uuid) return -1;
							if (b.dataset.uuid === playerData?.uuid) return 1;
							if (a.dataset.uuid === joinedPartyCache?.ownerUuid) return -1;
							if (b.dataset.uuid === joinedPartyCache?.ownerUuid) return 1;
							return baseFunc(a, b);
						};
			case 'friendsPlayerList':
				return (a, b) => {
					...
					if (typeof a.dataset.name !== 'string') return 0;
			...
			case 'partyModalOfflinePlayerList':
				return (a, b) => {
					const partyModalPartyId = document.getElementById('partyModal').dataset.partyId;
					if (a.dataset.uuid === partyCache[partyModalPartyId]?.ownerUuid) return -1;
					if (b.dataset.uuid === partyCache[partyModalPartyId]?.ownerUuid) return 1;
					if (a.dataset.uuid === playerData?.uuid) return -1;
					if (b.dataset.uuid === playerData?.uuid) return 1;
					const playerA = globalPlayerData[a.dataset.uuid];
					const playerB = globalPlayerData[b.dataset.uuid];
	...
	return new Promise((resolve) => {
		apiFetch('blocklist')
			...
			.then((jsonResponse) => {
				...
				if (updateModal) blocklistModalPlayerList.innerHTML = '';

				blockedPlayerUuids = [];

				Array.from(
					document.querySelectorAll(
						'.messageContainer.blockedHidden, .gameChatMessageContainer.blockedHidden'
					)
				).map((el) => el.classList.add('pendingUnblock'));
				...
				for (let blockedPlayer of blocklist) {
					...
					Array.from(
						document.querySelectorAll(
							`.messageContainer[data-sender-uuid='${blockedPlayer.uuid}'], .gameChatMessageContainer[data-sender-uuid='${blockedPlayer.uuid}']`
						)
					).map((el) => {
					...
					if (updateModal) {
						...
						updatePlayerListEntrySprite(
							blocklistModalPlayerList,
							blockedPlayer.spriteName,
							blockedPlayer.spriteIndex,
							blockedPlayer.uuid,
							blockedPlayer.game || ynoGameId
						);
					}
				}
				...
				Array.from(
					document.querySelectorAll(
						'.messageContainer.pendingUnblock, .gameChatMessageContainer.pendingUnblock'
					)
				).map((el) => {
				...
				if (updateModal)
					document
						.getElementById('blocklistModalEmptyLabel')
						.classList.toggle('hidden', !!blocklist.length);
...
function getLastOnlineInterval(date) {
	...
	if (timeDiffSeconds < 60) return localizedInterval.short;
	const timeDiffMinutes = Math.floor(timeDiffSeconds / 60);
	if (timeDiffMinutes < 60)
		return localizedInterval.minutes[timeDiffMinutes === 1 ? 'singular' : 'plural'].replace(
			'{VALUE}',
			timeDiffMinutes
		);
	const timeDiffHours = Math.floor(timeDiffMinutes / 60);
	if (timeDiffHours < 24)
		return localizedInterval.hours[timeDiffHours === 1 ? 'singular' : 'plural'].replace(
			'{VALUE}',
			timeDiffHours
		);
	const timeDiffDays = Math.floor(timeDiffHours / 24);
	return localizedInterval.days[timeDiffDays === 1 ? 'singular' : 'plural'].replace(
		'{VALUE}',
		timeDiffDays
	);
}

async function getSpriteProfileImg(
	sprite: string,
	idx,
	favicon = false,
	dir?: unknown,
	...
	if (!spriteData[cacheKey]) spriteData[cacheKey] = {};
	if (!spriteData[cacheKey][idx]) spriteData[cacheKey][idx] = null;
	...
	return await new Promise((resolve) => {
		...
		const getDefaultSpriteImg = new Promise((resolve) => {
			if (sprite !== defaultSprite || idx !== defaultIdx)
				getSpriteProfileImg(defaultSprite, defaultIdx, favicon, undefined, gameId).then(
					(defaultSpriteImg) => resolve(defaultSpriteImg)
				);
			else resolve(null);
		});
		if (!sprite || idx === -1)
		...
		if (!dir) {
			...
			img.onerror = () =>
				getSpriteProfileImg(sprite, idx, favicon, `images/charsets/${gameId}/`, gameId).then(
					(url) => resolve(url)
				);
		} else {
			img.onerror = () => {
...
function updatePlayerSprite(sprite, idx, gameId) {
	getSpriteProfileImg(sprite, idx, true, undefined, gameId).then(
		(faviconImg) => (document.getElementById('favicon').href = faviconImg)
	);
...
function getDefaultSprite() {
	if (typeof gameDefaultSprite === 'object') return gameDefaultSprite;
...
function onPlayerConnectedOrUpdated(systemName, name, id) {
	...
	if (uuid) {
		if (name) globalPlayerData[uuid].name = name;
		if (systemName) globalPlayerData[uuid].systemName = systemName;
	}
	setTimeout(() =>
...
function onPlayerSpriteUpdated(sprite, idx, id) {
	updatePlayerListEntrySprite(
		...
		sprite,
		idx,
		id !== -1 ? playerUuids[id] : defaultUuid,
		ynoGameId
	);
}
...
function showPlayerToastMessage(key, playerName, icon, iconFill, systemName, persist) {
	if (!notificationConfig.players.all || !notificationConfig.players[key]) return;
	const message = getMassagedLabel(localizedMessages.toast.players[key], true).replace(
		'{PLAYER}',
		playerName
	);
