<script>
	...
	let { schedule, isMod, gameId, joinedPartyId, localizedMessages, config, systemName, player, onEdit, onCancel } =
		$props();
	...
	function updateContent() {
		...
		if (schedule.datetime) {
			...
			datetimeText = parsedDatetime.toLocaleString(locale, {
				dateStyle: 'short',
				timeStyle: 'short'
			});
		}
		...
		for (const platform of platformAuthorities) {
			if (schedule[platform]) {
				...
				links.push(
					`<a target="_blank" href="javascript:void(0)">${localizedMessages.schedules.platforms[platform]}</a>`
				);
			}
		}
	...
	async function handleFollow() {
		...
		const response = await apiFetch(
			`schedule?command=follow&value=${!schedule.playerLiked}&scheduleId=${schedule.id}`
		);
		if (response.ok) {
	...
	<div class="listEntryMain">
		...
		<div class="themeText scheduleHeader">
			{#if isMod && (!player?.uuid || player.uuid === schedule.ownerUuid)}
				<button class="icon iconButton fillIcon unselectable" onclick={onEdit}>
				...
				<button class="deleteIcon icon iconButton" onclick={onCancel}>
			...
			<div class="likeContainer" style="display: flex; align-items: center;">
				<div class="likeIcon icon iconButton toggleButton altToggleButton" onclick={handleFollow}>
					<svg viewBox="0 0 18 18" width="24" height="24">
						<path
