			...
			if (preloadFiles) {
				...
				for (let preload of Object.entries(preloadFiles)) {
					for (let mapId of preload[0].split('/')) {
						if (!preloadFilesReplace.hasOwnProperty(mapId)) preloadFilesReplace[mapId] = [];
...
function preloadFileAndSave(link, languageLink, translatedFileName) {
	...
	if (gameLoadedFiles.has(link)) return;
...
function preloadFilesFromMapId(mapId) {
	if (!preloadFiles || !preloadFiles.hasOwnProperty(mapId)) return;
	let langLink;
	...
	for (let preloadFile of preloadFiles[mapId]) {
		if (typeof preloadFile === 'object') {
			...
			if (tpX === pfX && tpY === pfY && (!preloadFile.questionable || globalConfig.questionablePreloads)) {
				...
				for (let preloadFileInside of preloadFile.files) {
					if (gameLoadedFiles.has(preloadFileInside)) continue;
					if (
						preloadLocalizationFiles.hasOwnProperty(preloadsGameLang) &&
						preloadLocalizationFiles[preloadsGameLang].hasOwnProperty(preloadFileInside)
					) {
						...
						if (preloadFileInside.endsWith('.lmu')) {
							...
							else preloadFileAndSave(translatedFileName, langLink, translatedFileName);
							current++;
							if (current % totalAtOnce === 0) currentTimeout += delayAdd;
							translatedFileName = '';
							langLink = '';
						}
					} else {
					...
					else preloadFileAndSave(preloadFileInside, langLink, translatedFileName);
					current++;
					if (current % totalAtOnce === 0) currentTimeout += delayAdd;
				}
			}
		...
		if (!gameLoadedFiles.has(preloadFile)) {
			if (
				preloadLocalizationFiles.hasOwnProperty(preloadsGameLang) &&
				preloadLocalizationFiles[preloadsGameLang].hasOwnProperty(preloadFile)
			) {
				translatedFileName = preloadLocalizationFiles[preloadsGameLang][preloadFile];
				langLink = '/' + preloadsGameLang;
				if (preloadFile.endsWith('.lmu')) {
					preloadFileAndSave(
						'/Language/' + preloadsGameLang + translatedFileName,
						langLink,
						translatedFileName
					);
					translatedFileName = '';
...
function onRequestFile(_url) {
	...
	if (filePath.indexOf('Title/') !== -1) {
		...
		for (let prevFile of prevLoadedFiles) {
			if (prevFile.indexOf('Language/') !== -1) {
				...
				preloadsGameLang = folderFilePath.substring(
					0,
					folderFilePath.substring(1, folderFilePath.length).indexOf('/') + 1
				);
				break;
			}
		}
	...
	if (prevLoadedFiles.length > 4) prevLoadedFiles.shift();
