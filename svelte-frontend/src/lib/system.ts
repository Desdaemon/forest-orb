...
function setUiTheme(value, isInit) {
  ...
  const uiTheme = isAuto ? get(systemName) || getDefaultUiTheme() : value;
...
function setFontStyle(fontStyle, isInit) {
  ...
  const uiTheme = (isAuto ? get(systemName) : config.uiTheme) || getDefaultUiTheme();
  if (gameUiThemes.indexOf(uiTheme) === -1)
    return;
...
function getUiThemeOption(uiTheme) {
  ...
  if (isAuto)
    uiTheme = get(systemName) || getDefaultUiTheme();
