...
function parseFreeformMarkdown(msg) {
	...
	for (const syntax of extendedMarkdown) msg = msg.replace(syntax.p, syntax.r);
	return msg;
}
...
function addScheduleItem(schedule) {
	...
	let parsedDatetime;
	try {
		if (schedule.datetime) parsedDatetime = new Date(schedule.datetime);
	} catch {}
		...
		switch (slot.dataset.role) {
			...
			case 'edit':
				...
				if (schedule.game !== gameId) {
					...
					addTooltip(
						slot,
						localizedMessages.schedules.wrongEditGame.replace(
							'{GAME}',
							localizedMessages.games[schedule.game]
						)
					);
				} else slot.addEventListener('click', () => openScheduleEditModal(schedule));
				break;
			case 'cancel':
				if (!isMod && (!playerData?.uuid || playerData.uuid !== schedule.ownerUuid)) break;
				slot.classList.remove('hidden');
				slot.addEventListener('click', () => {
					showConfirmModal(localizedMessages.schedules.confirmCancel, () => {
						apiFetch(`schedule?command=cancel&scheduleId=${schedule.id}`).then(
							(response) => {
					...
					apiFetch(`schedule?command=follow&value=${!schedule.playerLiked}&scheduleId=${schedule.id}`)
						...
						.then(
							(likeCount) => {
								...
								if (elm) elm.innerText = likeCount;
							},
							(err) => console.error(err)
						);
				});
			...
			case 'links':
				...
				break;
			case 'organizer':
				const playerNameHtml = getPlayerName(
					{
					...
					if (schedule.recurring) {
						...
						if (schedule.interval === 1)
							switch (schedule.intervalType) {
								case 'days':
									addTooltip(slot, localizedMessages.schedules.intervals.perDay);
									break;
								case 'months':
									addTooltip(slot, localizedMessages.schedules.intervals.perMonth);
									break;
								case 'years':
									addTooltip(slot, localizedMessages.schedules.intervals.perYear);
									break;
							}
							...
							if (weeks == 1)
								addTooltip(
									slot,
									localizedMessages.schedules.intervals.perWeek.replace(
										'{WEEKDAY}',
										parsedDatetime.toLocaleString(locale, { weekday: 'long' })
									)
								);
							else
								addTooltip(
									slot,
									localizedMessages.schedules.intervals.weeks.replace('{INTERVAL}', String(weeks))
								);
						} else
							addTooltip(
								slot,
								localizedMessages.schedules.intervals[schedule.intervalType].replace(
									'{INTERVAL}',
									schedule.interval
								)
							);
	...
	if (schedule.systemName && schedule.game) {
		...
		if (!allGameUiThemes[schedule.game].includes(theme)) theme = getDefaultUiTheme(schedule.game);
	...
	fastdom.mutate(() => {
		...
		else if (schedule.official) document.getElementById('officialSchedules').appendChild(template);
		else document.getElementById('futureSchedules').appendChild(template);
	});
}
...
export function openSchedulesModal() {
	const schedulesModal = document.getElementById('schedulesModal');
	addLoader(schedulesModal);
	for (const elm of document.querySelectorAll('.scheduleContainer')) elm.replaceChildren();
		...
		.then(
			...
			(schedules) => {
				...
				for (const schedule of schedules)
					if (!schedule.partyId || schedule.partyId === joinedPartyId) addScheduleItem(schedule);
...
function openScheduleEditModal(schedule = {}) {
	...
	document
		.getElementById('restrictPartyRow')
		.classList.toggle('hidden', !joinedPartyCache || joinedPartyCache.ownerUuid !== playerData?.uuid);
	...
	if (!scheduleSystemName) scheduleSystemName = config.uiTheme === 'auto' ? systemName : config.uiTheme;
...
/** Toggles a checkbox-style button and reports its new state. */
...
/** Bound to `#eventRecurring` in `<ScheduleEditModal />`. */
export function onToggleEventRecurring(ev) {
	const enabled = toggleButton(ev.currentTarget);
...
/** Bound to the plain toggle buttons in `<ScheduleEditModal />`. */
export function onToggleScheduleButton(ev) {
	toggleButton(ev.currentTarget);
}

/** Bound to `#scheduleThemeButton` in `<ScheduleEditModal />`. */
export function onClickScheduleTheme(ev) {
	openModal('uiThemesModal', ev.currentTarget.nextElementSibling.value, 'scheduleEditModal');
}

/** Bound to the `[data-platform]` inputs in `<ScheduleEditModal />`. */
export function onChangePlatformLink(ev) {
	const input = ev.currentTarget;
	const sanitized = sanitizeLink(input.name, input.value, true);
	if (sanitized !== null) {
		input.value = sanitized;
		input.setCustomValidity('');
	} else input.setCustomValidity(localizedMessages.schedules.invalidPlatformLink);
}

/** Bound to `#scheduleForm`'s submit in `<ScheduleEditModal />`. */
export function editSchedule() {
	...
	if (
		editingScheduleId &&
		playerData.rank > 0 &&
		!document.getElementById('resetOrganizer').classList.contains('toggled')
	)
		schedule.ownerUuid = '';
...
/** Bound to `#cancelSchedule` in `<ScheduleEditModal />`. */
export function cancelSchedule() {
	...
	showConfirmModal(localizedMessages.schedules.confirmCancel, () => {
		apiFetch(`schedule?command=cancel&scheduleId=${editingScheduleId}`).then(
			(response) => {
				if (!response.ok) throw new Error(response.statusText);
				openSchedulesModal();
			},
