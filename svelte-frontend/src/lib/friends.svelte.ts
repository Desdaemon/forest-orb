...
export let playerFriendsCache = $state([]);
let pendingOfflineFriendUuids = [];

export function updatePlayerFriends() {
	if (loggedIn) sendSessionCommand('pf');
	else onUpdatePlayerFriends([]);
}
...
async function onUpdatePlayerFriends(playerFriends) {
	...
	for (let playerUuid of removedPlayerUuids) removePlayerListEntry(friendsPlayerList, playerUuid);
	...
	for (let playerFriend of playerFriends) {
		...
		if (oldFriendData) {
			...
			if (playerFriend.online !== oldFriendData.online) {
				if (playerFriend.online) {
					if (pendingOfflineFriendIndex > -1) pendingOfflineFriendUuids.splice(pendingOfflineFriendIndex, 1);
					else showFriendsToastMessage('playerOnline', 'friend', playerFriend);
				} else pendingOfflineFriendUuids.push(uuid);
		...
		if (playerFriend.badge === 'null') playerFriend.badge = null;
	...
	for (let playerFriend of playerFriends) {
		...
		entry.dataset.categoryId = playerFriend.accepted
			? playerFriend.online
				? 'online'
				: 'offline'
			: playerFriend.incoming
				? 'incoming'
				: 'outgoing';
	...
	['incoming', 'outgoing', 'online', 'offline'].forEach((c) =>
		updatePlayerListEntryHeader(friendsPlayerList, 'friends', c)
	);
	...
	const incomingFriendRequestCountLabel =
		incomingFriendRequestCountContainer.querySelector('.notificationCountLabel');
	...
	if (incomingCount) {
		...
		if (newIncomingCount) playersTabFriends.classList.toggle('unread', !!incomingCount);
...
export function showFriendsToastMessage(key, icon, player, persist) {
	if (!notificationConfig.friends.all || !notificationConfig.friends[key]) return;
	let message = getMassagedLabel(localizedMessages.toast.friends[key], true);
	if (player) message = message.replace('{PLAYER}', getPlayerName(player, true, false, true));
	showToastMessage(message, icon, true, null, persist);
}

/** Registers the friends session command handlers. Called from `<SessionCommands />`. */
export function registerFriendsSessionHandlers() {
