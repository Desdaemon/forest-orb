...
let _gameChatModeIndex = $state(0);
export const gameChatModeIndex = () => _gameChatModeIndex;

const mentionSe = isBrowser && new Audio('./audio/mention.wav');
...
function chatboxAddMessage(
	...
	if (msgId && messages.querySelector(`.messageContainer[data-msg-id="${msgId}"]`)) return null;
	...
	if (!system) {
		...
		if (global || party) {
			...
			if (showLocation) {
				...
				if (gameId === '2kki' && (!localizedMapLocations || !localizedMapLocations.hasOwnProperty(mapId))) {
					const prevLocations =
						prevLocationsStr && prevMapId !== '0000'
			...
			} else msgHeader.appendChild(document.createElement('span'));

			if (global) {
				chatTypeIcon = getSvgIcon('global', true);
				addTooltip(
					chatTypeIcon,
					document.createTextNode(getMassagedLabel(localizedMessages.chat.globalMessage)),
					true,
					true
				);
			} else {
				chatTypeIcon = getSvgIcon('party', true);
				if (joinedPartyCache) addTooltip(chatTypeIcon, getPartyName(joinedPartyCache, false, true), true, true);
			}
			messageSender.appendChild(chatTypeIcon);
		} else msgHeader.appendChild(document.createElement('span'));

		if (blockedPlayerUuids.indexOf(uuid) > -1) msgContainer.classList.add('blockedHidden');

		const defaultDate = !timestamp;
		if (defaultDate) timestamp = new Date();
		...
		addPlayerContextMenu(
			...
			player,
			uuid,
			global ? MESSAGE_TYPE.GLOBAL : party ? MESSAGE_TYPE.PARTY : MESSAGE_TYPE.MAP,
			{ msg, msgId }
		);
		...
		if (player?.rank) {
			...
			addTooltip(
				rankIcon,
				getMassagedLabel(localizedMessages.roles[Object.keys(localizedMessages.roles)[rank - 1]], true),
				true,
				true
			);
		...
		if (party) {
			...
			if (joinedPartyCache?.systemName) {
				...
				if (partyOwnerIcon) partyOwnerIcon.querySelector('path').setAttribute('style', iconStyle);
			}
		}
		...
		if (badge) {
			...
			if (player.name) {
				addOrUpdatePlayerBadgeGalleryTooltip(
					badgeEl,
					player.name,
					(systemName || getDefaultUiTheme()).replace(/'/g, ''),
					mapId,
					prevMapId,
					prevLocationsStr,
					x,
		...
		if (systemName) {
			...
			initUiThemeContainerStyles(systemName, null, false, () => {
				initUiThemeFontStyles(systemName, null, 0, false, () => {
					...
					if (rankIcon) applyThemeStyles(rankIcon, parsedSystemName);
					if (badgeOverlayEl) {
						...
						if (badgeOverlay2El) {
							...
							} else badgeOverlay2El.style.background = `var(--base-bg-color-${parsedSystemName})`;
						}
		...
		if (badgeEl) messageSender.appendChild(badgeEl);
	...
	if (playerName && new RegExp(`(^|[^a-z\d])@${playerName}($|[^a-z\d])`, 'i').test(msg)) {
		...
		if (globalConfig.playMentionSound && blockedPlayerUuids.indexOf(uuid) === -1) mentionSe.play();
	}
	...
	if (!messageContents.innerText.trim()) messageContents.classList.add('notext');
	...
	if (localizedMapLocations && !global) {
		...
		if (mapMessages.length) {
			...
			if (lastMapMessage.classList.contains('locMessage')) lastMapMessage.classList.remove('hidden');
		}
	}
	...
	if (shouldScroll) task = Math.abs(messages.scrollHeight - messages.scrollTop - messages.clientHeight) <= 60;

	const didPopulateMessage = task.then(() => {
		messages.appendChild(msgContainer);
	});
	Promise.allSettled([didPopulateMessage, systemThemeProm]).then(() => {
		if (player) addGameChatMessage(message.innerHTML, type, uuid);
	});
	...
	if (!ignoreNotify) {
		if ((globalChat || partyChat) && (system || map)) document.getElementById('chatTabMap').classList.add('unread');
		else if ((mapChat || partyChat) && global) document.getElementById('chatTabGlobal').classList.add('unread');
		else if ((mapChat || globalChat) && party) document.getElementById('chatTabParty').classList.add('unread');
			...
			if (!unreadMessageCount || unreadMessageCount < 9)
				unreadMessageCountLabel.textContent =
					++unreadMessageCount < 9 ? unreadMessageCount : `${unreadMessageCount}+`;
		}
	}

	let tabMessagesLimit;

	if (global) tabMessagesLimit = parseInt(globalConfig.globalChatHistoryLimit);
	else if (party) tabMessagesLimit = parseInt(globalConfig.partyChatHistoryLimit);
	else tabMessagesLimit = parseInt(globalConfig.mapChatHistoryLimit);

	if (tabMessagesLimit) {
		let tabMessages;
		if (global) tabMessages = [...messages.querySelectorAll('.messageContainer.global')];
		else if (party) tabMessages = [...messages.querySelectorAll('.messageContainer.party')];
		else tabMessages = [...messages.querySelectorAll('.messageContainer:not(.global):not(.party)')];
		const oldTask = task;
		while (tabMessages.length > tabMessagesLimit) tabMessages.shift().replaceWith(tabMessages[0]);
		return oldTask;
	}
...
function addGameChatMessage(messageHtml, messageType, senderUuid) {
	...
	if (messageType === 2) {
		if (!globalConfig.gameChatGlobal) messageContainer.classList.add('hidden');
	} else if (messageType === 3) {
		if (!globalConfig.gameChatParty || !joinedPartyId) messageContainer.classList.add('hidden');
	}
	messageContainer.dataset.messageType = messageType;
	messageContainer.dataset.senderUuid = senderUuid;

	if (blockedPlayerUuids.indexOf(senderUuid) > -1) messageContainer.classList.add('blockedHidden');
	...
	{
		gameChatContainer.insertBefore(
			messageContainer,
			gameChatContainer.children[gameChatContainer.childElementCount - 1]
		);

		const typeMessages = Array.from(gameChatContainer.children).filter((m) => m.dataset.messageType == messageType);
		if (typeMessages.length > 10) typeMessages[0].remove();
	}
...
export function setGameChatMode(modeIndex) {
	...
	_gameChatModeIndex = modeIndex;
	if (modeIndex) {
		if (modeIndex === 1) {
			if (globalConfig.gameChatGlobal) chatModeIcon.innerHTML = getSvgIcon('global', true).outerHTML;
			else cycleGameChatMode();
		} else {
			if (globalConfig.gameChatParty && joinedPartyId)
				chatModeIcon.innerHTML = getSvgIcon('party', true).outerHTML;
			else cycleGameChatMode();
		}
	} else chatModeIcon.innerHTML = '';
	document
		.getElementById('gameChatInputContainer')
		.querySelector('.globalCooldownIcon')
		.classList.toggle('hidden', modeIndex !== 1);
}

export function cycleGameChatMode() {
	if (_gameChatModeIndex < 2) setGameChatMode(_gameChatModeIndex + 1);
	else setGameChatMode(0);
}
...
function chatInputActionFired() {
	...
	if (!chatInput?.value.trim().length) return;
	const htmlTextEl = document.createElement('span');
	htmlTextEl.innerHTML = parseMessageTextForMarkdown(chatInput.value);
	if (!htmlTextEl.innerText.trim().length) return;
	const partyChat = document.getElementById('chatbox').classList.contains('partyChat');
	if (!chatInput.dataset.global && !partyChat && connStatus != 1 && connStatus != 3) return;
	if (chatInput.dataset.global && chatInput.dataset.blockGlobal) return;
	...
	if (message.includes('[screenshot]') && chatInput.dataset.screenshotId) {
		...
		message = message.replace(
			'[screenshot]',
			`[${chatInput.dataset.screenshotTemp ? 't' : ''}${chatInput.dataset.screenshotId}${flags ? `:${flags}` : ''}]`
		);
	...
	if (!chatInput.dataset.global || partyChat) {
		if (!joinedPartyId || !partyChat) {
			if (!trySendMapMessage(message)) return;
		} else {
			if (!trySendPartyMessage(message)) return;
		}
	} else if (!trySendGlobalMessage(message)) return;
	chatInput.value = '';
...
export function trySetChatName(name: string) {
	...
	if (playerName) {
		...
		if (!loggedIn) sendSessionCommand('name', [playerName]);
	}
}
...
export function trySendPartyMessage(content: string) {
	const chatInputContainer = document.getElementById('chatInputContainer');
	if (!chatInputContainer.classList.contains('globalCooldown')) {
		const chatInputContainers = [chatInputContainer, document.getElementById('gameChatInputContainer')];
...
export function trySendGlobalMessage(content) {
	const chatInput = document.getElementById('chatInput');
	if (chatInput.blockGlobal) return false;
...
export function addChatTip() {
	...
	if (++globalConfig.chatTipIndex >= Object.keys(tips).length) globalConfig.chatTipIndex = 0;
	const tipIndex = globalConfig.chatTipIndex;
	const tipKey = Object.keys(tips)[tipIndex];
	const msgContainer = chatboxAddMessage(
		getMassagedLabel(localizedMessages.chatTips.template.replace('{CONTENT}', tips[tipKey])),
		null,
		null,
		true
	);
...
export function addChatMapLocation(locations) {
	...
	if (
		lastLocMessage &&
		new DOMParser().parseFromString(locationHtml, 'text/html').documentElement.textContent ===
			lastLocMessage.innerText
	)
		return;
	...
	const prevLocationsStr =
		gameId === '2kki' && typeof cachedPrev2kkiLocations !== 'undefined' && cachedPrev2kkiLocations?.length
...
function getChatMessageTimestampLabel(timestamp, defaultDate) {
	const timeString = timestamp.toLocaleString(globalConfig.lang === 'en' ? [] : globalConfig.lang, {
		timeStyle: 'short'
	});
	const weekdayString =
		!defaultDate && new Date().toDateString() !== timestamp.toDateString()
			? timestamp.toLocaleString(globalConfig.lang === 'en' ? [] : globalConfig.lang, { weekday: 'short' })
			: null;

	let timestampLabel = getMassagedLabel(
		localizedMessages.timestamp[weekdayString ? 'timeAndWeekday' : 'time'],
		true
	).replace('{TIME}', timeString);
	if (weekdayString) timestampLabel = timestampLabel.replace('{WEEKDAY}', weekdayString);
...
export async function syncChatHistory() {
	...
	const chatHistory = await apiFetch(
		`chathistory?globalMsgLimit=${globalConfig.globalChatHistoryLimit}&partyMsgLimit=${globalConfig.partyChatHistoryLimit}${lastMessageId ? `&lastMsgId=${lastMessageId}` : ''}`
	).then((response) => {
	...
	if (chatHistory.messages) {
		...
		for (let message of chatHistory.messages) {
			chatboxAddMessage(
				message.contents,
				message.party ? MESSAGE_TYPE.PARTY : MESSAGE_TYPE.GLOBAL,
				message.uuid,
				true,
				message.mapId,
				message.prevMapId,
				message.prevLocations,
				message.x,
				message.y,
				message.msgId,
...
export function parseMessageTextForMarkdown(msg: string) {
	for (let syntax of markdownSyntax) msg = msg.replace(syntax.p, syntax.r);
...
export function populateMessageNodes(msg, node, asHtml) {
	...
	while ((result = tagPattern.exec(msg.slice(cursor)))) {
		if (result.index) {
			...
			} else textNode = document.createTextNode(content);
...
export function wrapMessageEmojis(node, force) {
	if (node.childNodes.length && !force) {
		for (let childNode of node.childNodes) {
			if (
				/\p{Extended_Pictographic}/u.test(childNode.textContent) ||
				/:([a-z0-9_\-]+):/i.test(childNode.textContent)
			) {
				...
				} else wrapMessageEmojis(childNode);
			}
		}
	} else {
		node.innerHTML = node.innerHTML.replace(/(\p{Extended_Pictographic}+)/gu, '<span class="emoji">$1</span>');
			...
			if (childNode.nodeType === Node.TEXT_NODE) {
				...
				if ((screenshotResult = screenshotPattern.exec(childNode.textContent)) !== null) {
					...
					imageNode.classList.toggle(
						'screenshotBlur',
						globalConfig.blurScreenshotEmbeds || !!(flags & SCREENSHOT_FLAGS.SPOILER)
					);
					...
					if (beforeNode) beforeNode.textContent = childNode.textContent.slice(0, screenshotResult.index);
					const afterNode =
						childNode.textContent.length > screenshotResult.index + offset
							? document.createTextNode('')
							: null;
					if (afterNode) afterNode.textContent = childNode.textContent.slice(screenshotResult.index + offset);
					node.replaceChild(imageNode, childNode);
					if (beforeNode) node.insertBefore(beforeNode, imageNode);
					if (afterNode) {
						if (childNode !== node.childNodes[node.childElementCount - 1])
							node.insertBefore(afterNode, imageNode.nextSibling);
						else node.appendChild(afterNode);
					}

					return true;
				}
			} else if (tryEmbedScreenshot(childNode, uuid)) return true;
		}
...
/** Registers the chat session command handlers. Called from `<SessionCommands />`. */
export function registerChatSessionHandlers() {
	...
	addSessionCommandHandler('psay', (args) => {
		...
		if (partyMember)
			chatboxAddMessage(
				...
				false,
				partyMember.mapId,
				partyMember.prevMapId,
				partyMember.prevLocations,
				partyMember.x,
				partyMember.y,
				msgId
			);
