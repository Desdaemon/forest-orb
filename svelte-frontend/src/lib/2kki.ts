import { addChatMapLocation } from '$lib/chat.svelte';
...
export function send2kkiApiRequest<T = unknown>(url: string, callback: (_: T) => unknown) {
	if (pendingRequests.hasOwnProperty(url)) pendingRequests[url].push(callback);
	else {
		...
		let onReqEnd = (_e) => {
			...
			if (Array.isArray(callbacks)) {
				for (let cb of callbacks) cb(req.response);
			}
...
export function onLoad2kkiMap(mapId) {
	...
	if (locations && typeof locations === 'string') locations = null;
	...
	if (locations && locations.length) {
		...
		if (!locationNames) {
			...
			else queryAndSet2kkiMaps(locationNames).catch((err) => console.error(err));
		...
		queryAndSet2kkiLocation(mapId, prevMapId, prevLocations, set2kkiClientLocation, true)
			.then((locations) => {
				...
				set2kkiExplorerLinks(locationNames);
				if (locationNames) queryAndSet2kkiMaps(locationNames).catch((err) => console.error(err));
				else {
...
function queryAndSet2kkiLocation(
	...
	return new Promise((resolve, reject) => {
		...
		if (!setLocationFunc) setLocationFunc = () => {};
		...
		send2kkiApiRequest<{}[]>(url, (response) => {
			...
			if (Array.isArray(locationsArray) && locationsArray.length) {
				...
				if (usePrevLocations) locations.push(location);
				else {
					for (let l of locationsArray) locations.push(l);
				}

				if (usePrevLocations) {
					queryConnected2kkiLocationNames(
						location.title,
						locationsArray.filter((l) => l.title !== location.title).map((l) => l.title)
					)
						.then((connectedLocationNames) => {
							const connectedLocations = locationsArray.filter(
								(l) => connectedLocationNames.indexOf(l.title) > -1
							);
							for (let cl of connectedLocations) locations.push(cl);
					...
					return;
				} else setLocationFunc(mapId, prevMapId, locations, prevLocations, true, true);
			} else {
				...
				if (prevMapId) {
					queryAndSet2kkiLocation(mapId, null, null, setLocationFunc, forClient)
						.then(() => resolve(null))
						.catch((err) => reject(err));
					return;
				}
		...
		setLocationFunc(
			mapId,
			prevMapId,
			getMassagedLabel(localizedMessages.location.queryingLocation),
			prevLocations,
			true
		);
	});
...
function set2kkiClientLocation(
	...
	if (cacheLocation) {
		...
		if (saveLocation && (locations || prevLocations)) {
			if (locations) setCacheValue(CACHE_TYPE.location, locationKey, locations);
			if (cachePrev) setCacheValue(CACHE_TYPE.location, prevLocationKey, prevLocations);
...
function getLocalized2kkiLocation(title, titleJP, asHtml, forDisplay = false) {
	...
	if (asHtml) template = template.replace(/}([^{]+)/g, '}<span class="infoLabel">$1</span>');
	return getMassagedLabel(template)
		.replace('{LOCATION}', title)
		.replace('{LOCATION_JP}', titleJP || '');
}
...
export function get2kkiLocationHtml(location, showDepth) {
	...
	if (location.connType) {
		...
		for (let ct of connTypes) {
			if (location.connType & ct) {
				...
				const localizedTooltip = localizedMessages.location.connType[ct]
					.replace('{PARAMS}', connTypeParams?.params || '')
					.replace('{PARAMS_JP}', connTypeParams?.paramsJP || connTypeParams?.params || '');
	...
	if (showDepth && location.hasOwnProperty('depth'))
		locationHtml += localizedMessages.location.depth.replace(
			'{DEPTH}',
			`<span class="locationDepth colorText" style="-webkit-text-fill-color: rgba(${getDepthRgba(location.depth, 10)})">${location.depth}</span>`
		);
...
export function getOrQuery2kkiLocationsHtml(mapId, callback) {
	...
	if (locationCache && !locationCache.hasOwnProperty(locationKey)) {
		const locationKeys = Object.keys(locationCache).filter(
			(k) => k.endsWith(mapId) && Array.isArray(locationCache[k])
		);
		if (
			locationKeys.length === 1 ||
			locationKeys
				.map((k) => JSON.stringify(locationCache[k]))
...
function queryConnected2kkiLocationNames(locationName: string, connLocationNames: string[]) {
	return new Promise((resolve, _reject) => {
		...
		send2kkiApiRequest(url, (response) => {
			...
			if (Array.isArray(response)) ret = response;
...
export function queryAndSet2kkiMaps(locationNames: string[]) {
	return new Promise<void>((resolve, _reject) => {
		...
		send2kkiApiRequest(url, (response) => {
			...
			if (Array.isArray(response)) setMaps(response, locationNames, true, true);
			else errCode = response?.err_code;
...
export function get2kkiWikiLocationName(location: string | {}) {
	...
	if (location.urlTitle)
		locationName = location.urlTitle
			.replace(/%26/g, '&')
			.replace(/%27/g, "'")
			.replace(/\_/g, ' ')
			.replace(/#.*/, '');
	...
	if (match1 != null && match2 != null) {
		...
		if (verNum1 === verNum2) {
			...
			if (subVer1 !== undefined && subVer2 !== undefined) {
				...
				if (subVer1 !== subVer2) return subVer1 < subVer2 ? -1 : 1;
			} else if (subVer2 != null) return -1;
			else if (subVer1 != null) return 1;
			...
			if (patchVer1 !== undefined && patchVer2 !== undefined) {
				...
				if (patchVer1 !== patchVer2) return patchVer1 < patchVer2 ? -1 : 1;
			} else if (patchVer2 != null) return -1;
			else if (patchVer1 != null) return 1;
		} else return verNum1 < verNum2 ? -1 : 1;
		if (match1[1] !== undefined) {
			if (match2[1] === undefined) return -1;
		} else if (match2[1] !== undefined) return 1;
	} else if (match2 != null) return -1;
	else if (match1 != null) return 1;
...
export function checkShow2kkiVersionUpdate() {
	return new Promise((resolve) => {
		...
		if (versionPrefixText.endsWith(' ')) versionPrefix.innerHTML += '&nbsp;';
...
function reloadExplorer(trackedLocations?: unknown[]) {
	if (!config.enableExplorer) return;
	...
	if (explorerFrame && locationNames && loggedIn) {
		...
		apiFetch(
			`explorer${trackedLocations ? `?trackedLocations=${trackedLocations.map((l) => l.title).join('|')}` : ''}`
		)
			.then((response) => {
				...
				if (explorerFrame.src !== newUrl) {
					...
					apiFetch('explorercompletion')
						...
						.then((textResponse) => {
							...
							document
								.getElementById('explorerUndiscoveredLocationsLink')
								.classList.toggle('hidden', !completionPercent || completionPercent < 95);
						});
				} else removeLoader(explorerFrame);
			})
			.catch((err) => console.error(err));
	}
}
