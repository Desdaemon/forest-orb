...
export function onUpdateEventPeriod(eventPeriod) {
	if (!eventPeriod || eventPeriod.periodOrdinal < 0) return;

	eventPeriod.endDate = new Date(eventPeriod.endDate);
	document.getElementById('eventPeriod').innerHTML = getMassagedLabel(
		localizedMessages.events.period.replace('{ORDINAL}', eventPeriod.periodOrdinal),
		true
	);
	document.getElementById('eventPeriodEndDateLabel').innerHTML = getMassagedLabel(
		localizedMessages.events.periodEnds.replace(
			'{DATE}',
			eventPeriod.endDate.toLocaleString(globalConfig.lang === 'en' ? [] : globalConfig.lang, {
				dateStyle: 'short',
				timeStyle: 'short'
			})
		),
...
export function updateEvents() {
	if (!loggedIn || !eventPeriodCache) return;
	...
	for (let eventType of eventTypes) {
		...
		for (let event of eventsCache[eventType]) {
			if (lastType === undefined || event.type !== lastType) {
				...
				eventLocationTypeLabel.innerHTML = getMassagedLabel(
					eventType === 'locations'
						? localizedMessages.events.types[event.type]
						: localizedMessages.events.current,
					true
				);
			...
			if (event.complete) eventListEntry.classList.add('complete');
			...
			if (eventType === 'locations') {
				...
				if (gameLink) detailsContainer.appendChild(gameLink);

				eventLocationName.innerHTML =
					eventGameId === '2kki'
						? get2kkiLocationHtml(event)
						: gameLocationsMap[eventGameId].hasOwnProperty(event.title)
							? getLocalizedLocation(
									eventGameId,
									gameLocalizedLocationsMap[eventGameId][event.title],
									gameLocationsMap[eventGameId][event.title],
									true
								)
							: event.title;
				...
				const getEventLocationDepthContainer = (isOutline, isMin) => {
					...
					if (isOutline) eventLocationDepth.classList.add('depthOutlineContainer');
					else {
						eventLocationDepth.classList.add('depthFillContainer');
						if (isMin) eventLocationDepth.classList.add('minDepthFillContainer');
					}
					...
					for (let d = 0; d < 10; d += 2) {
						let starIcon;
						if (isOutline) starIcon = getSvgIcon('star');
							...
							if (d + 1 < event[depthProp]) {
								...
								halfStarPath.setAttribute(
									'style',
									`fill: var(--${gameId === eventGameId ? 'modal-svg-base-gradient' : `svg-base-gradient-${eventGameId}-${getDefaultUiTheme(eventGameId)}`})`
								);
								if (isMin) starSvg.querySelector('path').remove();
				...
				if (event.minDepth) {
					...
					addTooltip(
						eventLocationDepth,
						getMassagedLabel(localizedMessages.events.shortcut, true),
						true,
						true
					);
				}
				...
				if (gameId === eventGameId && eventGameId === '2kki' && !event.complete) {
					...
					trackButton.innerHTML =
						'<svg viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" width="28" height="28"><path d="m9 0a1 1 0 0 0 0 18 1 1 0 0 0 0-18m0 2a1 1 0 0 1 0 14 1 1 0 0 1 0-14m4 11-2-6-6-2 2 6 6 2m-4-5a1 1 0 0 1 0 2 1 1 0 0 1 0-2"></path></svg>';

					trackButton.onclick = function () {
						const toggled = config.trackedLocationId !== event.locationId;
						if (toggled)
							Array.from(document.querySelectorAll('.eventLocationTrackButton.toggled')).map((t) =>
						...
						if (toggled) sendSessionCommand('nl', [event.locationId]);
						updateConfig(config);
					};
				...
				if (gameLink) vmContainer.appendChild(gameLink);
			...
			pointsContainer.innerHTML = getMassagedLabel(localizedMessages.events.exp, true).replace(
				'{POINTS}',
				event.exp || 0
			);
			...
			if (event.complete) checkbox.classList.add('toggled');
		...
		if (
			eventType === 'locations' &&
			!ignoreLocationCheck &&
			(connStatus === 1 || connStatus === 3 || connStatus === 4)
		)
			checkEventLocations();
	}
...
function onUpdatePlayerExp(exp) {
	...
	document.getElementById('totalExp').innerHTML = getMassagedLabel(
		localizedMessages.events.exp.replace('{POINTS}', exp.totalExp),
		true
	);
...
export function checkEventLocations() {
	if (loggedIn && cachedLocations && eventsCache.locations?.length) {
		...
		if (eventLocationMatch)
			setTimeout(
				() =>
					claimEventLocationPoints(
						eventLocationMatch,
						incompleteEventLocations.find((el) => el.title === eventLocationMatch).type === -1
					),
				1000
			);
	}
...
function showEventsToastMessage(key, icon, location, exp) {
	if (!notificationConfig.events.all || !notificationConfig.events[key]) return;
