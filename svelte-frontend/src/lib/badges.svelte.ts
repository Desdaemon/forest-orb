import { gameId, isBrowser, hasTouchscreen } from '$lib';
import { apiFetch } from '$lib/init';
import { handleBadgeOverlayLocationColorOverride } from '$lib/play.svelte';
...
type BadgeCache = Badge[] & { full?: boolean };
...
let badgeCache: BadgeCache | null;
/** row-major */
let badgeSlotCache: string[][];
let badgePresetCache;
...
let localizedBadges: Record<string, Record<string, any>>;
...
// Resolved on first use: `<BadgeGalleryModal />` is not in the document when this module loads.
let badgeGalleryModalContent;
...
/**
 * Badge UI that the modals render declaratively. The badge lists themselves are still built
 * imperatively, so anything the list code has to look up stays a plain DOM query below.
 */
export const badgeUi = $state({
	/** Set while the modal picks a badge for a gallery slot: `#badgeGalleryButton` is hidden then. */
	modifyingSlot: false,
	/** `''` unless a search is active, otherwise the searched field. */
	searchMode: '',
	dropdownOpen: false,
	/** Remove mode of the badge gallery. */
	removing: false
});

/** The sort orders rendered as `#badgeSortOrder` options by `<BadgesModal />`. */
export const badgeSortOrderNames = Object.keys(badgeSortOrderTypes);

// The badge modals are mounted by the time any of this runs, but not when the module is
// imported, so the elements are resolved on first use.
let badgeModalContent;
let badgeGameTabs;
let badgeCategoryTabs;
let sortOrder;
let badgeSearch;
let badgeDropdown;

function resolveElements() {
	badgeModalContent ||= document.querySelector('#badgesModal .modalContent');
	badgeGameTabs ||= document.getElementById('badgeGameTabs');
	badgeCategoryTabs ||= document.getElementById('badgeCategoryTabs');
	sortOrder ||= document.getElementById('badgeSortOrder');
	badgeSearch ||= document.getElementById('badgeSearch');
...
async function fetchAndUpdateBadgeModalBadges(slotRow?, slotCol?, searchMode?) {
	...
	if (slotRow && slotCol) modifyingSlot = { slotRow, slotCol };
	else modifyingSlot = null;
	if (searchMode) currentSearchMode = searchMode;
	...
	badgeCompareFunc = (a, b) => {
		if (sortOrderType) return badgeSortOrderTypes[sortOrderType](a, b, sortOrderDesc);
		if (a.game !== b.game) {
			if (a.game === 'ynoproject') return -1;
			if (b.game === 'ynoproject') return 1;
			if (a.game === gameId) return -1;
			if (b.game === gameId) return 1;
	...
	if (userSelectedSortOrder) sortOrderType = userSelectedSortOrder;
	gameBadges = {};
	const spacePattern = / /g;
	let badgeCount = 0;
	badgeObserver?.disconnect();
	badgeObserver = new IntersectionObserver(
		(observed) => {
	...
	for (const badge of playerBadges) {
		...
		if (!gameBadges[badge.game]) {
			...
			if (systemName) {
				...
				initUiThemeContainerStyles(parsedSystemName, badge.game, false, () =>
					initUiThemeFontStyles(parsedSystemName, badge.game, 0)
				);
			}
		...
		if (badge.badgeId === (playerData?.badge || 'null')) item.children[0].classList.add('selected');
	...
	if ('null' === (playerData?.badge || 'null')) nullBadge.children[0].classList.add('selected');
	if (!nullBadge.classList.contains('disabled')) nullBadge.onclick = () => onClickBadge('null');
	...
	function updateBadges(game, group) {
		...
		if (game !== 'ynoproject') {
			...
			for (const cls of nullBadge.classList) if (cls.startsWith('theme_')) nullBadge.classList.remove(cls);
		}
		...
		tab.onclick = () => {
			...
			window.requestAnimationFrame(() => {
				...
				for (const game in gameBadges)
					for (const category in gameBadges[game]) badgeModalContent.append(...gameBadges[game][category]);
				badgeTabGame = 'all';
				badgeTabGroup = null;
			});
		}; // tab.onclick
				...
				subTab.onclick = () => {
					...
					if (game !== 'ynoproject') {
						...
						for (const cls of nullBadge.classList)
							if (cls.startsWith('theme_')) nullBadge.classList.remove(cls);
					}

					{
						badgeModalContent.replaceChildren(nullBadge);
						for (const group in gameBadges[game]) badgeModalContent.append(...gameBadges[game][group]);
					}
					badgeTabGame = game;
					badgeTabGroup = 'all';
				};
	...
	didUpdateBadgeModal = async () => {
		...
		} else await updateBadgeVisibility();
		removeLoader(document.getElementById('badgesModal'));
	};
	if (userSelectedSortOrder) updateBadgeModalOnly();
	else await didUpdateBadgeModal();
}
...
const updateBadgeModalOnly = () => {
	...
	setTimeout(async () => {
		for (let idx = 0; idx < cacheIndexes.length; idx++) badgeFilterCache[cacheIndexes[idx]].el.style.order = idx;
...
const updateBadgesAndPopulateModal = (slotRow, slotCol) => {
	resolveElements();
	badgeUi.modifyingSlot = !!(slotRow && slotCol);
	...
	for (let sot of Object.keys(badgeSortOrderTypes)) {
		...
		optionAsc.innerHTML = getMassagedLabel(
			localizedMessages.badges.sortOrder.template
				.replace('{TYPE}', localizedMessages.badges.sortOrder.types[sot])
				.replace('{ORDER}', localizedMessages.badges.sortOrder.asc)
		);
		optionDesc.innerHTML = getMassagedLabel(
			localizedMessages.badges.sortOrder.template
				.replace('{TYPE}', localizedMessages.badges.sortOrder.types[sot])
				.replace('{ORDER}', localizedMessages.badges.sortOrder.desc)
		);
	}
...
/** Bound to `#badgeSortOrder` in `<BadgesModal />`. Re-sorts whichever list is open. */
export function onChangeBadgeSortOrder() {
	if (!badgeCache?.full) addLoader(document.getElementById('badgesModal'), true);
	updateBadgesAndPopulateModal(modifyingSlot?.slotRow, modifyingSlot?.slotCol);
}

const onClickBadgeButton = (prevModal?, slotRow?, slotCol?) => {
	if (slotRow && slotCol && (slotRow > badgeSlotRows || slotCol > badgeSlotCols)) return;
	...
	if (
		!badgeCache.filter((b) => localizedBadges?.[b.game]?.hasOwnProperty(b.badgeId)).length ||
		localizedBadgesIgnoreUpdateTimer
	)
		updateBadgesAndPopulateModal(slotRow, slotCol);
	else updateLocalizedBadges(() => updateBadgesAndPopulateModal(slotRow, slotCol));
};

/** Bound to `#badgeButton` / `#fsBadgesButton` in `+page.svelte`. */
export function openBadgesModal() {
	onClickBadgeButton();
}

/** Bound to `#accountBadgeButton` in `<AccountSettingsModal />`. */
export function openBadgesModalFromAccount() {
	onClickBadgeButton('accountSettingsModal');
}

let currentSearchMode = 'name';
/** Bound to `#badgeUnlockStatus` in `<BadgesModal />`. */
export const updateBadgeVisibility = (searchMode?) => {
	if (typeof searchMode !== 'string') searchMode = currentSearchMode;
	else currentSearchMode = searchMode;
	...
	for (let item of badgeFilterCache) {
		{
			let visible = true;
			if (unlockStatus === 'recentUnlock') visible &= newUnlockBadges.has(item.badgeId);
			else if (unlockStatus !== '') visible &= item.el.classList.contains('locked') === !parseInt(unlockStatus);
			if (searchTerm.trim().length) {
				switch (searchMode) {
					case 'name':
						visible &=
							item.title &&
							(exactMatch ? item.title === parsedSearchTerm : item.title.indexOf(parsedSearchTerm) > -1);
						break;
					case 'location': {
						...
						visible &=
							title &&
							(exactMatch
								? title.toLocaleLowerCase() === parsedSearchTerm
								: title.toLocaleLowerCase().indexOf(parsedSearchTerm) > -1);
						break;
					}
					case 'artist': {
						visible &=
							item.art &&
							(exactMatch
								? item.art.split(/ & |, /).includes(parsedSearchTerm)
								: item.art.indexOf(parsedSearchTerm) > -1);
						break;
					}
				}
			}
			...
			if (visible) {
				if (!gameVisibilities[item.game]) gameVisibilities[item.game] = true;
		...
		for (let header of badgeModalContent.querySelectorAll('.itemCategoryHeader'))
			header.classList.toggle(
				...
				!(header.dataset.group
					? gameGroupVisibilities[header.dataset.game][header.dataset.group]
					: gameVisibilities[header.dataset.game])
			);
	}
};

/** Bound to `#badgeSearch` in `<BadgesModal />`. */
export function onInputBadgeSearch(ev) {
	resolveElements();
	const value = ev.currentTarget.value;
	badgeUi.dropdownOpen = !!value;
	if (!value) {
		searchBadges();
		return;
	}
	...
	for (const span of badgeDropdown.querySelectorAll('.dropdownItem span')) {
		...
		if (modifier)
			value += localizedMessages.badges.search.modifier.template.replace(
				'{MODIFIER}',
				localizedMessages.badges.search.modifier[modifier]
			);
...
/** Bound to `#badgeSearch` in `<BadgesModal />`. */
export function onKeydownBadgeSearch(ev) {
	if (ev.key !== 'Enter') return;
	searchBadges('name');
}

/** Bound to `#badgeSearch` and its dropdown items in `<BadgesModal />`. */
export function hideBadgeDropdown() {
	resolveElements();
	setTimeout(() => {
		if (!badgeSearch.parentElement.querySelector('.dropdownItem:focus')) badgeUi.dropdownOpen = false;
	}, 60);
}

/** Bound to `#badgeSearch` in `<BadgesModal />`. */
export function onFocusBadgeSearch(ev) {
	ev.currentTarget.select();
	badgeUi.dropdownOpen = !!ev.currentTarget.value;
}

/**
 * Bound to the search dropdown items in `<BadgesModal />`; they double as buttons, so a click
 * and an Enter keypress both activate them.
 * @param {'name' | 'location' | 'artist'} mode
 */
export function onActivateSearchMode(mode) {
	return function (ev) {
...
function searchBadges(mode?) {
	badgeUi.dropdownOpen = false;
	badgeUi.searchMode = mode || '';
	updateBadgeVisibility(mode);
}

/** Bound to `#badgeSearchClearLink` in `<BadgesModal />`. */
export function clearBadgeSearch() {
	resolveElements();
	badgeSearch.value = '';
	searchBadges();
}

/** Bound to `#badgeGalleryButton` in `<BadgesModal />`. */
export function openBadgeGalleryModal() {
	updateBadgeSlots(() => {
		initBadgeGalleryModal();
...
/** Bound to `#badgePresetButton` in `<BadgeGalleryModal />`. */
export function openBadgePresetModal() {
	initBadgePresetModal();
	openModal('badgePresetModal', undefined, 'badgeGalleryModal');
}

/** Bound to `#badgePresetSave` in `<BadgePresetModal />`. */
export function saveBadgePreset() {
	apiFetch(`badge?command=presetSave&preset=${document.getElementById('badgePresetSelection').value}`).then(
		(response) => {
			if (response.ok) updateCurrentPreset(badgeSlotCache);
			else throw new Error(response.statusText);
		}
	);
}

/** Bound to `#badgePresetLoad` in `<BadgePresetModal />`. */
export function loadBadgePreset() {
	apiFetch(`badge?command=presetLoad&preset=${document.getElementById('badgePresetSelection').value}`).then(
		(response) => {
			if (!response.ok) throw new Error(response.statusText);
			badgeSlotCache = badgePresetCache;
			badgePresetCache = null;
			initBadgeGalleryModal();
			closeModal();
		}
	);
...
let draggedBadge;
/**
 * Bound to the badge slots in `<BadgeGalleryModal />` for both `dragstart` and `dragend`.
 * @param {DragEvent} ev
 */
export function didDragBadgeSlot(ev) {
...
/**
 * Bound to the badge slots in `<BadgeGalleryModal />` for both `dragover` and `dragleave`.
 * @param {DragEvent} ev
 */
export function mightDropOnBadgeSlot(ev) {
	ev.preventDefault();
	if (this.classList.contains('dragging')) return;
	this.classList.toggle('dropTarget', ev.type === 'dragover');
}

/** Bound to the badge slots in `<BadgeGalleryModal />`. */
export function didDropOnBadgeSlot() {
 ...
 * @param {MouseEvent} ev
 */
export async function onClickBadgeSlot(ev) {
	let { row, col } = this.dataset;
	row = +row;
	col = +col;
	if (badgeUi.removing || ev.shiftKey) {
...
/**
 * Bound to the badge slots in `<BadgeGalleryModal />` for `mouseenter`, `mousemove` and
 * `mouseleave`; shift-hovering a filled slot previews its removal.
 * @param {MouseEvent} ev
 */
export function highlightRemoveBadgeSlot(ev) {
...
/** The badge gallery grid, rendered by `<BadgeGalleryModal />`. */
export const badgeSlotRowNumbers = Array.from({ length: maxBadgeSlotRows }, (_, r) => r + 1);
export const badgeSlotColNumbers = Array.from({ length: maxBadgeSlotCols }, (_, c) => c + 1);

/** Bound to `#removeBadgesButton` in `<BadgeGalleryModal />`. */
export function toggleRemoveBadges() {
	badgeUi.removing = !badgeUi.removing;
	this.innerHTML = badgeUi.removing
		? getMassagedLabel(i18next.t('modal.badgeGallery.removeMode.deactivate', 'Done'))
		: getMassagedLabel(i18next.t('modal.badgeGallery.removeMode.activate', 'Remove Badges'));
}
...
function initBadgeGalleryModal() {
	...
	for (let rl = 0; rl < badgeGalleryRowBpLevels.length; rl++) {
		...
		if (totalBp < rowBpLevel.bp) break;
	...
	for (let cl = 0; cl < badgeGalleryColBcLevels.length; cl++) {
		...
		if (totalBc < colBcLevel.bc) break;
	...
	document.getElementById('badgeGalleryTotalBp').innerHTML = getMassagedLabel(
		localizedMessages.badgeGallery.bp.replace('{BP}', totalBp),
		true
	);
	rootStyle.setProperty('--col-level-total-bc', levelColBc);
	rootStyle.setProperty('--col-level-bc', totalBc - prevLevelColBc);
	document.getElementById('badgeGalleryTotalBc').innerHTML = getMassagedLabel(
		localizedMessages.badgeGallery.count.replace('{COUNT}', totalBc),
		true
	);

	badgeGalleryModalContent ||= document.querySelector('#badgeGalleryModal .modalContent');

	for (let r = 1; r <= maxBadgeSlotRows; r++) {
		for (let c = 1; c <= maxBadgeSlotCols; c++) {
			const badgeId =
				r <= badgeSlotCache.length && c <= badgeSlotCache[r - 1].length ? badgeSlotCache[r - 1][c - 1] : null;
			const badgeSlotButton = badgeGalleryModalContent.querySelector(
				`.badgeSlotButton[data-row='${r}'][data-col='${c}']`
			);
			if (badgeSlotButton) {
				let badge = badgeId && badgeId !== 'null' ? badgeCache.find((b) => b.badgeId === badgeId) : null;
				if (!badge) badge = { badgeId: 'null' };
				...
				if (badge?.overlayType & BadgeOverlayType.LOCATION)
					handleBadgeOverlayLocationColorOverride(
						badgeSlotButton.querySelector('.badgeOverlay'),
						badgeSlotButton.querySelector('.badgeOverlay2'),
						cachedLocations
					);
			}
		}
...
/** Bound to `#badgePresetSelection` in `<BadgePresetModal />`. */
export function initBadgePresetModal() {
	...
	apiFetch(`badge?command=presetGet&preset=${document.getElementById('badgePresetSelection').value}`)
		...
		.finally(() => {
			removeLoader(document.getElementById('badgePresetModal'));
		});
}
...
function isEmptyBadgeSlots(badgeSlots: string[][] | undefined) {
	...
	for (const row of badgeSlots) for (const badgeId of row) if (badgeId !== 'null') return false;
...
function updateCurrentPreset(preset: string[][]) {
	...
	if (isEmptyBadgeSlots(preset)) {
		...
		span.innerText = localizedMessages.badgePreset.empty;
		items.replaceChildren(span);
		return;
	}
	const badgeSlots = formatBadgeContainer(preset);
	badgeSlots.classList.add('listEntry');
...
export function updateBadgeButton() {
	...
	if (badge?.overlayType & BadgeOverlayType.LOCATION)
		handleBadgeOverlayLocationColorOverride(
			badgeButton.querySelector('.badgeOverlay'),
			badgeButton.querySelector('.badgeOverlay2'),
			cachedLocations
		);
}
...
export function getBadgeItem(badge, includeTooltip, emptyIcon, lockedIcon, scaled, filterable, parsedSystemName, lazy) {
	...
	if (badge.hidden && badge.unlocked) badgeContainer.classList.add('special');
	...
	if (badgeEl) {
		...
		if (scaled) badgeEl.classList.add('scaledBadge');
		setBadgeBackgroundImage = () => {
			...
			if (badge.overlayType) {
				...
				if (badge.overlayType & BadgeOverlayType.MULTIPLY) badgeOverlay.classList.add('badgeOverlayMultiply');

				badgeEl.appendChild(badgeOverlay);

				const badgeMaskValue =
					badge.overlayType & BadgeOverlayType.MASK
						? `url('${badgeUrl.replace('.', badge.overlayType & BadgeOverlayType.DUAL ? '_mask_fg.' : '_mask.')}')`
						: badgeEl.style.backgroundImage;

				badgeOverlay.setAttribute(
					'style',
					`-webkit-mask-image: ${badgeMaskValue}; mask-image: ${badgeMaskValue};`
				);

				if (badge.overlayType & BadgeOverlayType.DUAL) {
					const badgeMask2Value =
						badge.overlayType & BadgeOverlayType.MASK
					...
					badgeOverlay2.classList.add(
						getStylePropertyValue('--base-color') !== getStylePropertyValue('--alt-color')
							? 'badgeOverlayAlt'
							: 'badgeOverlayBg'
					);
					...
					badgeOverlay2.setAttribute(
						'style',
						`-webkit-mask-image: ${badgeMask2Value}; mask-image: ${badgeMask2Value};`
					);
				}
			}
		};
		...
		if (!badge.unlocked) {
			...
			if (lockedIcon) badgeContainer.appendChild(getSvgIcon('locked', true));
		}
	} else if (badgeId !== 'null') {
		item.classList.add('locked', 'disabled');
		if (lockedIcon) badgeContainer.appendChild(getSvgIcon('locked', true));
		badgeContainer.appendChild(document.createElement('div'));
	} else badgeContainer.appendChild(emptyIcon ? getSvgIcon('ban', true) : document.createElement('div'));

	if (parsedSystemName) applyThemeStyles(item, parsedSystemName, badge.game);

	if (includeTooltip) {
		let tooltipContent = '';

		if (badgeId === 'null') tooltipContent = `<label>${localizedMessages.badges.null}</label>`;
		else {
			if (localizedBadges.hasOwnProperty(badge.game) && badgeId in localizedBadges[badge.game]) {
				...
				if (badge.bp)
					badgeTitle = getMassagedLabel(localizedMessages.badges.badgeTitle)
						.replace('{TITLE}', badgeTitle)
						.replace('{BP}', badge.bp);
						...
						if (localizedTooltip.checkbox && badge.tags?.length)
							for (const subcondition in localizedTooltip.checkbox) {
								...
								if (subcondition.includes('|'))
									subconditionAchieved = !!subcondition
										.split('|')
										.find((tag) => badge.tags.includes(tag));
								else subconditionAchieved = badge.tags.includes(subcondition);
								if (subconditionAchieved) condition = condition.replace(needle, `<tag>${needle}</tag>`);
							}
						tooltipContent += `<div class="tooltipContent">${condition}</div>`;
					} else tooltipContent += `<h3 class="tooltipTitle">${localizedMessages.badges.locked}</h3>`;
				}
			} else {
				tooltipContent += `<h3 class="tooltipTitle">${localizedMessages.badges.locked}</h3>`;
				if (filterItem) filterItem.title = localizedMessages.badges.locked;
			}
			...
			if (tooltipContent) {
				...
				const assignTooltipOrDefer = (instance) => {
					...
					const assignImmediately = () => {
						const badgeTippy = addOrUpdateTooltip(
							item,
							tooltipContent,
							false,
							false,
					...
					if (lazy) assignTooltip = assignImmediately;
					else assignImmediately();
				};
				...
				if (badge.mapId) {
					...
					const setTooltipLocation = (instance) => {
						if (badge.game in gameLocalizedMapLocations && mapId in gameLocalizedMapLocations[badge.game])
							tooltipContent = baseTooltipContent.replace(
								'{LOCATION}',
								getLocalizedMapLocationsHtml(
									badge.game,
									mapId,
									'0000',
									badge.mapX,
							...
							tooltipContent = baseTooltipContent.replace(
								'{LOCATION}',
								getInfoLabel(getMassagedLabel(localizedMessages.location.queryingLocation))
							);
							tooltipOptions.onCreate = (instance) =>
								getOrQuery2kkiLocationsHtml(mapId, (locationsHtml) =>
									instance.setContent(baseTooltipContent.replace('{LOCATION}', locationsHtml))
								);
						} else
							tooltipContent = baseTooltipContent.replace(
								'{LOCATION}',
					...
					if (badge.game in gameLocalizedMapLocations) setTooltipLocation();
					else {
						tooltipContent = baseTooltipContent.replace(
							'{LOCATION}',
							getInfoLabel(getMassagedLabel(localizedMessages.location.queryingLocation))
						...
						tooltipOptions.onCreate = (instance) => {
							includeTooltip = true;
							if (badge.game in gameLocalizedMapLocations) setTooltipLocation(instance);
							else
								fetchAndInitLocations(globalConfig.lang, badge.game).then(() =>
									setTooltipLocation(instance)
								);
						};
						assignTooltipOrDefer();
					}
				} else assignTooltipOrDefer();
			}
		}
	}
	...
	if (lazy) {
		didObserveBadgeCallbacks.set(item, () => {
			if (setBadgeBackgroundImage) fastdom.mutate(setBadgeBackgroundImage);
			assignTooltip?.();
		});
	} else {
		setBadgeBackgroundImage?.();
...
function fetchPlayerBadges() {
	if (badgeCache?.full) return badgeCache;
	return apiFetch('badge?command=list')
		...
		.then(
			(badges) => {
				...
				badgeCache = badges;
				for (let i = 0; i < badgeCache.length; ++i) badgeCache[i].originalOrder = i;
...
export function updateBadges(callback?: Function) {
	apiFetch('badge?command=list&simple=true')
		...
		.then((badges) => {
			...
			if (badgeCacheUpdateTimer) clearInterval(badgeCacheUpdateTimer);
...
function checkNewBadgeUnlocks() {
	const now = Date.now();
	if (isBadgeCheckPending || now - lastBadgeCheckTime < 1000) return Promise.resolve();
	...
	return apiFetch(`badge?command=new&since=${lastBadgeCheck}`)
		...
		.then((checkData) => {
			...
			if (checkData.badgeIds?.length || checkData.newTags) {
				...
				if (checkData.badgeIds) {
					for (const badgeId of checkData.badgeIds) {
						if (!newUnlockBadges.has(badgeId)) updateBadgeHints = true;
						newUnlockBadges.add(badgeId);
						showBadgeToastMessage('badgeUnlocked', 'info', badgeId);
					}
				}
			...
			for (const badge of badgeCache) {
				if (newUnlockBadges.has(badge.badgeId)) badge.unlocked = true;
			}
			if (updateBadgeHints) updateBadgeHint(cachedLocations.map((l) => l.title));
		})
		.catch((err) => console.error(err))
		.finally(() => {
...
function updateLocalizedBadges(callback) {
	if (localizedBadgesIgnoreUpdateTimer) clearInterval(localizedBadgesIgnoreUpdateTimer);
			...
			onShow: (instance) => {
				...
				apiFetch(`badge?command=playerSlotList&player=${playerName}`)
					...
					.then((badgeSlots) => {
						...
						tooltipTitle.innerHTML = getMassagedLabel(localizedMessages.badgeGallery.label, true).replace(
							'{PLAYER}',
							playerName
						);
						...
						const parsedSystemName = systemName
							? (gameUiThemes.indexOf(systemName) > -1 ? systemName : getDefaultUiTheme()).replace(
									/ /g,
									'_'
								)
							: null;

						if (parsedSystemName) applyThemeStyles(tippyBox, parsedSystemName);

						const badgeSlotRowsContainer = formatBadgeContainer(badgeSlots);
						...
						if (parsedSystemName) {
							...
							for (let badgeSlotOverlay of badgeSlotOverlays) {
								...
								badgeSlotOverlay.style.background =
									overlayType & BadgeOverlayType.GRADIENT
										? `var(--base-gradient-${parsedSystemName})`
										: `rgb(var(--base-color-${parsedSystemName}))`;

								const badgeSlotOverlay2 =
									overlayType & BadgeOverlayType.DUAL
										? badgeSlotOverlay.parentElement.querySelector('.badgeOverlay2')
										: null;

								if (badgeSlotOverlay2) {
									if (
										getStylePropertyValue(`--base-color-${parsedSystemName}`) !==
										getStylePropertyValue(`--alt-color-${parsedSystemName}`)
									) {
										badgeSlotOverlay2.style.background =
											overlayType & BadgeOverlayType.GRADIENT
						...
						if (localizedBadges) {
							...
							for (let badge of badges) {
								...
								if (badgeGame) {
									const badgeTippy = addTooltip(
										badge,
										getMassagedLabel(localizedBadges[badgeGame][badgeId].name, true),
										true,
										false,
										true
									);
...
function formatBadgeContainer(badgeSlots: string[][]) {
	...
	for (let r = 0; r < badgeSlots.length; ++r) {
		const badgeRowSlots = badgeSlots[r];
		let rowHasBadge = false;
		for (let c = 0; c < badgeRowSlots.length; ++c) {
			if (badgeRowSlots[c] === 'null') continue;
			rowHasBadge = true;
			if (c < minColIndex) minColIndex = c;
			if (c > maxColIndex) maxColIndex = c;
		}
		if (rowHasBadge) {
			if (r < minRowIndex) minRowIndex = r;
	...
	for (let r = 0; r < badgeSlots.length; ++r) {
		if (r < minRowIndex || r > maxRowIndex) continue;
		const badgeRowSlots = badgeSlots[r];
		...
		for (let c = 0; c < badgeRowSlots.length; ++c) {
			if (c < minColIndex || c > maxColIndex) continue;
			const badgeId = badgeRowSlots[c];
			...
			if (badgeId !== 'null') {
				...
				if (badge?.overlayType) {
					...
					badgeSlotOverlay.setAttribute(
						'style',
						`-webkit-mask-image: url('${badgeMaskUrl}'); mask-image: url('${badgeMaskUrl}');`
					);
					...
					if (badge.overlayType & BadgeOverlayType.DUAL) {
						...
						const badgeMask2Url =
							badge.overlayType & BadgeOverlayType.MASK ? badgeUrl.replace('.', '_mask_bg.') : badgeUrl;

						badgeSlotOverlay2.setAttribute(
							'style',
							`-webkit-mask-image: url('${badgeMask2Url}'); mask-image: url('${badgeMask2Url}');`
						);
					}
				}
			}
...
function showBadgeToastMessage(key, icon, badgeId) {
	if (
		!notificationConfig.badges.all ||
		(notificationConfig.badges.hasOwnProperty(key) && !notificationConfig.badges[key])
	)
		return;
...
export function setUpTwoFingerPan(element: Element, contentElement?) {
