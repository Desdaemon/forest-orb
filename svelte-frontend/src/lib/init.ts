export const gameIds = ['2kki', 'amillusion', 'braingirl', 'unconscious', 'deepdreams', 'flow', 'fog', 'genie', 'if', 'mikan', 'muma', 'nostalgic', 'oversomnia', 'oneshot', 'prayers', 'sheawaits', 'someday', 'tsushin', 'ultraviolet', 'unaccomplished', 'unevendream', 'yume'] as const;
...
export function inferGameId(payload: InitPayload = {}): GameId {
	...
	if (isBrowser) {
		...
		if (query && gameIds.includes(query as GameId))
			return query as GameId;
	}
	return '2kki';
}
