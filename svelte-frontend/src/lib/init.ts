...
export let loadedLang = false;
export let loadedUiTheme = false;
export let loadedFontStyle = false;
...
export function wikiApiFetch(action, query) {
	return new Promise((resolve, reject) => {
		if (!yumeWikiSupported) reject('Game not supported by yume.wiki');
...
export async function getSpriteImg(
	img,
	spriteData,
	sprite,
	idx,
	...
	return new Promise((resolve) => {
		...
		for (let i = 0; i < data.length; i += 4) {
			if (checkPixelTransparent(data, i, transPixel)) data[i + 3] = 0;
			else if (yOffset === -1) yOffset = Math.max(Math.min(i >> 7, 15), 3);
		}
		...
		extractCanvas.toBlob((blob) => {
			...
			if (Array.isArray(spriteData[idx])) spriteData[idx][frameIdx] = url;
			else spriteData[idx] = url;
			resolve(url);
		});
	});
}
...
let transparencyChecker = {
	default: (data, o, transPixel) =>
		data[o] === transPixel[0] && data[o + 1] === transPixel[1] && data[o + 2] === transPixel[2],
	brave: (data, o, transPixel) =>
		(data[o] === transPixel[0] || data[o] - 1 === transPixel[0]) &&
		(data[o + 1] === transPixel[1] || data[o + 1] - 1 === transPixel[1]) &&
		(data[o + 2] === transPixel[2] || data[o + 2] - 1 === transPixel[2])
};
...
export function addTooltip(
	...
	if (delayed) options.delay = [750, 0];
	if (!asTooltipContent) options.content = content;
	...
	} else options.content = `<div class="tooltipContent">${content}</div>`;
...
export function addPlayerContextMenu(
	...
	if (player && !player.hasOwnProperty('uuid')) player = Object.assign({ uuid }, player);
	...
	target.addEventListener('contextmenu', function (event) {
		...
		if (addFriendAction) addFriendAction.classList.toggle('hidden', isFriend);
		if (removeFriendAction) removeFriendAction.classList.toggle('hidden', !isFriend);
		...
		if (blockAction) blockAction.classList.toggle('hidden', isBlocked);
		if (unblockAction) unblockAction.classList.toggle('hidden', !isBlocked);
		...
		playerTooltip.popper
			.querySelector('.grantBadgeAction')
			?.classList.toggle('hidden', !(isMod && targetHasAccount));
		playerTooltip.popper
			.querySelector('.revokeBadgeAction')
			?.classList.toggle('hidden', !(isMod && targetHasAccount));
...
function createPlayerTooltip(target, player, uuid, messageType, msgProps) {
		...
		playerTooltip.popper.querySelector('.addPlayerFriendAction').onclick = function () {
			...
			apiFetch(`addplayerfriend?uuid=${uuid}`)
				...
				.then((_) => {
					...
					} else showFriendsToastMessage('accept', 'approve', player);
					updatePlayerFriends();
				})
					...
					if (cachedPlayerFriend) {
						if (cachedPlayerFriend.accepted)
							...
							showFriendsToastMessage(
								cachedPlayerFriend.incoming ? 'reject' : 'cancel',
								'deny',
								cachedPlayerFriend
							);
	...
	if (isMod) {
		...
		playerTooltip.popper.querySelector('.tempbanPlayerAction').onclick = function () {
			const expiry = prompt(
				localizedMessages.context.admin.tempban.prompt.replace('{PLAYER}', playerNamePlain),
			...
			apiFetch(`tempban?${new URLSearchParams({ expiry, uuid })}`, true)
				...
				.then((_) =>
					showToastMessage(
						getMassagedLabel(localizedMessages.context.admin.tempban.success, true).replace(
							'{PLAYER}',
							playerName
						),
						'ban',
		...
		playerTooltip.popper.querySelector('.tempmutePlayerAction').onclick = function () {
			const expiry = prompt(
				localizedMessages.context.admin.tempmute.prompt.replace('{PLAYER}', playerNamePlain),
				new Date().toISOString()
			);
			if (!expiry) return;
			apiFetch(`tempmute?${new URLSearchParams({ expiry, uuid })}`, true)
				...
				.then((_) =>
					showToastMessage(
						getMassagedLabel(localizedMessages.context.admin.tempmute.success, true).replace(
							'{PLAYER}',
							playerName
						),
						'mute',
				...
				if (badgeId) {
					...
					if (badgeGame) {
						...
						apiFetch(`${isGrant ? 'grant' : 'revoke'}badge&uuid=${uuid}&id=${badgeId}`, true)
							...
							.then((_) =>
								showToastMessage(
									getMassagedLabel(localizedContextRoot.success, true)
										.replace('{BADGE}', badgeName)
							...
							.catch((err) => console.error(err));
					} else alert(localizedContextRoot.fail);
				}
			};
		}
	}
					...
					try {
						...
						switch (configName) {
								...
								if (global) {
									switch (key) {
										case 'lang':
											...
											if (langElement) langElement.value = value;
											loadedLang = true;
											break;
										case 'name':
											setName(value, true);
											const nameInputElement = document.getElementById('nameInput');
											if (nameInputElement) nameInputElement.value = value;
											break;
										case 'soundVolume':
											setSoundVolume(value, true);
											const soundVolumeElement = document.getElementById('soundVolume');
											if (soundVolumeElement) soundVolumeElement.value = value;
											break;
										case 'musicVolume':
											setMusicVolume(value, true);
											const musicVolumeElement = document.getElementById('musicVolume');
											if (musicVolumeElement) musicVolumeElement.value = value;
											break;
										case 'wikiLinkMode':
											setWikiLinkMode(value, true);
											const wikiLinkModeElement = document.getElementById('wikiLinkMode');
											if (wikiLinkModeElement) wikiLinkModeElement.value = value;
											break;
										case 'saveReminder':
											setSaveReminder(value, true);
											const saveReminderElement = document.getElementById('saveReminder');
											if (saveReminderElement) saveReminderElement.value = value;
											break;
										case 'gameChat':
											if (value) document.getElementById('gameChatButton').click();
											break;
										case 'gameChatGlobal':
											if (value) document.getElementById('gameChatGlobalButton').click();
											break;
										case 'gameChatParty':
											if (value) document.getElementById('gameChatPartyButton').click();
											break;
										case 'tabToChat':
											if (!value) document.getElementById('tabToChatButton').click();
											break;
										case 'badgeHints':
											if (!value) document.getElementById('badgeHintsButton').click();
											break;
										case 'playBadgeHintSound':
											if (!value) document.getElementById('playBadgeHintSoundButton').click();
											break;
										case 'playMentionSound':
											if (!value) document.getElementById('playMentionSoundButton').click();
											break;
										case 'blurScreenshotEmbeds':
											if (value) document.getElementById('blurScreenshotEmbedsButton').click();
											break;
										case 'mapChatHistoryLimit':
											setMapChatHistoryLimit(value, true);
											const mapChatHistoryLimitElement =
												document.getElementById('mapChatHistoryLimit');
											if (mapChatHistoryLimitElement) mapChatHistoryLimitElement.value = value;
											break;
										case 'globalChatHistoryLimit':
											setGlobalChatHistoryLimit(value, true);
											const globalChatHistoryLimitElement =
												document.getElementById('globalChatHistoryLimit');
										...
										case 'mobileControlsType':
											...
											if (mobileControlElement) mobileControlElement.value = value;
											break;
										case 'locationDisplay':
											if (value) document.getElementById('locationDisplayButton').click();
											break;
										case 'hideRankings':
											if (value) document.getElementById('toggleRankingsButton').click();
											break;
										case 'hideSchedules':
											if (value) document.getElementById('toggleSchedulesButton').click();
											break;
										case 'autoDownloadScreenshots':
											if (value) document.getElementById('autoDownloadScreenshotsButton').click();
											break;
										case 'screenshotResolution':
											setScreenshotResolution(value, true);
											const screenshotResolutionElement =
												document.getElementById('screenshotResolution');
											if (screenshotResolutionElement) screenshotResolutionElement.value = value;
											break;
										case 'preloads':
											if (value) {
												initPreloadList();
										...
										case 'questionablePreloads':
											...
											break;
										case 'unicodeFont':
											if (value) document.getElementById('toggleUnicodeFont').click();
											break;
										case 'rulesReviewed':
											break;
									...
									switch (key) {
										case 'privateMode':
											if (value) preToggle(document.getElementById('privateModeButton'));
											break;
										case 'singleplayerMode':
											if (value) preToggle(document.getElementById('singleplayerModeButton'));
											break;
										case 'disableChat':
											if (value) document.getElementById('chatButton').click();
											break;
										case 'explorer':
											if (value && gameId === '2kki')
										...
										case 'nametagMode':
											...
											if (nametagModeElement) nametagModeElement.value = value;
											break;
										case 'disablePlayerSounds':
											if (value) preToggle(document.getElementById('playerSoundsButton'));
											break;
										case 'hideLocation':
											...
											break;
										case 'enableExplorer':
											if (value) document.getElementById('enableExplorerButton').click();
											break;
										case 'immersionMode':
											if (value) document.getElementById('immersionModeButton').click();
											break;
										case 'mute':
											if (value) preToggle(document.getElementById('muteButton'));
											break;
										case 'chatTabIndex':
											if (value) {
										...
										case 'playersTabIndex':
											if (value) {
												const playersTab = document.querySelector(
													`.playersTab:nth-child(${value + 1})`
												);
												if (playersTab && value !== 1) setPlayersTab(playersTab);
											}
											break;
										case 'globalMessage':
											if (value) document.getElementById('globalMessageButton').click();
											break;
										case 'hideGlobalMessageLocations':
											if (value) document.getElementById('globalMessageLocationsButton').click();
											break;
										case 'hideMessageTimestamps':
											if (value) document.getElementById('messageTimestampsButton').click();
											break;
										case 'filterMentions':
											if (value) document.getElementById('mentionFilterButton').click();
											break;
										case 'hideUnnamedPlayers':
											if (value) document.getElementById('hideUnnamedPlayersButton').click();
											break;
										...
										case 'uiTheme':
											if (gameUiThemes.indexOf(value) > -1) {
												...
												if (uiThemeElement) uiThemeElement.value = value;
												loadedUiTheme = true;
											}
											break;
										case 'fontStyle':
											if (gameUiThemes.indexOf(value) > -1) {
												...
												if (fontStyleElement) fontStyleElement.value = value;
							...
							case 'notificationConfig':
								switch (key) {
									case 'all':
										if (!value) document.getElementById('notificationsButton').click();
										break;
									case 'screenPosition':
										if (value && value !== 'bottomLeft') setNotificationScreenPosition(value);
										break;
									default:
